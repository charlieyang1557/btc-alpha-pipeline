...
def build_cohort_csv(rows: list[dict], cohort_dir: Path) -> pd.DataFrame:
    ...
    Args:
        ...
        cohort_dir: Root path for Path B artifacts; CSV written as
...
def load_pathb_moments(
    ...
    gamma3/gamma4/T recompute + lineage guard. Path B never bypasses this gate.

    Args:
        hashes: List of hypothesis_hash strings to load.
        df: The holdout_results.csv DataFrame (from build_cohort_csv).
        cohort_dir: Root path for Path B artifacts (same as in build_cohort_csv).
