# backtest/patha_moments.py
"""Path A moments: assemble the cohort CSV from producer rows, then load each
CandidateMoments through the EXISTING tier6 integrity gate (Decision 3).

Adapted near-verbatim from backtest/pathb_moments.py; retargeted to the Path A
funding cohort namespace. We do NOT recompute moments here —
load_candidate_moments performs the sha256 verification + the independent
gamma3/gamma4/T recompute, so Path A's funding moments meet exactly the same
integrity bar as the dead-18 cohort they are DSR-compared against. Path A owns
...
def build_cohort_csv(rows: list[dict], cohort_dir: Path) -> pd.DataFrame:
    """Assemble + persist the Path A holdout_results.csv from producer rows.
    ...
    Args:
        ...
        cohort_dir: Root path for Path A artifacts; CSV written as
...
def load_patha_moments(
    hashes: list[str], df: pd.DataFrame, cohort_dir: Path
) -> list[CandidateMoments]:
    """Load each funding candidate's moments via the integrity-gated tier6 loader.

    Reuses ``load_candidate_moments`` (Decision 3): sha256 + independent
    gamma3/gamma4/T recompute + lineage guard. Path A never bypasses this gate.

    Args:
        hashes: List of hypothesis_hash strings to load.
        df: The holdout_results.csv DataFrame (from build_cohort_csv).
        cohort_dir: Root path for Path A artifacts (same as in build_cohort_csv).
