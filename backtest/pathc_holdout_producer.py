# backtest/pathc_holdout_producer.py
"""Path C holdout producer: run one basis candidate on a window -> dead-18-layout
...
Adapted near-verbatim from backtest/patha_holdout_producer.py; retargeted to the
Path C basis cohort (theme defaults to the caller's "pathc"; default
current_git_sha = "PATHC_BUILD"). Writes ONLY into a Path C namespace; never the
sealed cohort dirs. The engine call is injected (default backtest.engine.run_backtest)
for test isolation.

IMPL NOTES (verified against backtest/wf_lineage.py — identical to Path B/A):
...
tier6_dsr_v1/ cohort; Path C owns its own namespace. The forward features must be
fully warmed — basis features are built on FULL history then sliced to the
window (full-dataset-build rule), enforced upstream in factors.build_features, so
forward_2026 basis columns are not NaN inside the window.

WARMUP-FEED PREPEND (2-leg-review HIGH): run_backtest crops the feed to
[start, end] (fromdate/todate), and the compiled basis strategy enforces a
...
window; the basis axis's ~2160-bar warmup makes the prepend necessary here.)
"""
...
def produce_candidate_holdout(
    *,
    ...
    current_git_sha: str = "PATHC_BUILD",
    _run_backtest: Callable[..., Any] = run_backtest,
    _write_per_bar: Callable[..., Any] = write_per_bar_artifact,
) -> dict[str, Any]:
    """Run one basis candidate on ``window`` and emit dead-18-layout artifacts.

    Inputs:
        hypothesis_hash: DSL hash string (also the sub-dir name).
        name: Strategy name (e.g. "pathc_h1").
        theme: Theme label (e.g. "pathc").
        ...
        cohort_dir: Root path for Path C artifacts; writes to cohort_dir/<hash>/.
        execution_config_path: Repo-relative YAML path for the cost anchor
            (e.g. "config/execution_phaseb_spot_15bps.yaml").
        current_git_sha: Git SHA to stamp; defaults to "PATHC_BUILD" for the build phase.
    ...
    # from the compiled strategy, NOT hardcoded) so the basis warmup completes
