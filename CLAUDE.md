...
- **Current phase:** Path A funding-rate axis mechanism-first mine — **VERDICT: `process_refuted_for_this_grid` (earned negative); A-escalation WARRANTED. Charlie-accepted binding read 2026-05-31** — on branch `patha-funding-scoping` (~42 ahead of `main`, **UNPUSHED**; `finishing-a-development-branch` pending), 2026-05-31 UTC. The Charlie-registered fresh scoping cycle (Path B's escalation-warranted successor; anti-pre-emption — warranted ≠ authorized) is **COMPLETE**: funding-only (Q1); N\*=3 (Q2); Tier-5 gate forward_2026 (Q3); all 3 long/flat (Q5); Step −1 LOCK + Amendment A1 (H1 no time-stop — pre-data correction of a mis-applied Path-B max_hold; §36.2-disclosed not-outcome-invariant) + Clarification C1 (half-open sizing band). **Build:** Phase A funding ingestion (Binance Vision bulk → `data/raw/btcusdt_funding_8h.parquet`, 6852 rows 2020-01..2026-04, 8h cadence, 1 real gap; validator gap-fix from a 4208-false-gap A6 finding — Binance calc_time ±ms jitter); Phase B 5 funding factors + 8h→1h causal carry (bar-CLOSE keyed) + `input_source`/`input_period_bars` routing; Phase C1-3 H1/H2/H3 DSL builders; C4-C7 `patha_*` verdict harness. **forward_2026 RUN** (2026-01-01..2026-04-16, 2528 bars, 15bps spot anchor; features full-history-warmed): H1 funding_extreme_fade **−1.77** (9 trades), H2 funding_sign_regime_switch **−2.98** (54), H3 funding_momentum_continuation **−1.62** (21). **0/3 Tier-5; 0/3 DSR pass_B** (deflated_z −1.81/−2.42/−1.71, PSR ≈0.04). All 3 tier5=INDETERMINATE (under TRAIN floors: H1 150<200 episodes; H2/H3 zero_fraction 0.62/0.67>0.50) — **but the negatives hold INDEPENDENT of floors** (measured loss, not eligibility exclusion; §37.3). Funding-marginal (fenced): H1 −0.68 (funding hurt), H2 −0.11 (inert), H3 **+0.38** (funding helped but on a still-losing book → "price-trend wearing a funding mask" prior confirmed). Mechanism-sanity (train): H1 **refuted** (extreme funding → positive fwd return, opposite the reversal hypothesis), H2/H3 strong-sane → any_mechanism_sane=True → process-refuted. **Localization:** "funding adds no RESCUE under this grid" — NOT "funding dead in general" (funding-not-exonerated; short legs / continuous-funding-scaled sizing / OI / basis cross-structure untried; symmetric to Path B's OHLCV caveat). Magnitude modest (deflated_z ~−2, not Path B's −8) → not over-leaning on "more conclusive." **Integrity event (disclosed for provenance):** an accidental real forward_2026 run executed during a fix-subagent's RED test (single-layer gate; §37.1); artifact on-disk-only, NEVER committed, no values leaked/seen → **NO peek** (verified); gate hardened. Every phase boundary 2-leg-B2'd (Codex + code-reviewer/advisor); Codex caught the deep bugs (§37.4); verdict result B2: Codex COMPUTATIONALLY SOUND + advisor SOUND-WITH-CAVEATS; Codex stalled on the wiring B2 (known failure mode) → adjudicated via code-reviewer + own verification. **Commits** `6373b48`(spec)→`c9fa744`(verdict) [full chain in history]; sealed `tier6_dsr_v1/` sha256 4/4 byte-unchanged throughout (incl. through the real run); canonical verdict artifact `data/phase2c_evaluation_gate/patha_verdict_v1/patha_verdict_advisory.json`. pc9 baseline 2602→2780. Local-only (no project Anthropic-API spend). METHODOLOGY_NOTES §37 codifies this arc's lessons. **Active next:** (i) `superpowers:finishing-a-development-branch` — Charlie's merge/PR/keep decision (~42 commits, unpushed; **public repo → pre-push secret/abspath grep first**); (ii) the NEXT crypto-native axis (OI / basis / liquidations / short legs / continuous-funding-scaled sizing) = a SEPARATE future Charlie register (anti-pre-emption); (iii) prior carry-forward backlog (Phase 4 paper-trading; Phase 2.5 bandit-dedup parked; abspath sanitize; pre-existing noise) — separate register-events. **Arc tag `patha-funding-mine-verdict-v1`** (proposed). _Prior harness-arc Phase Marker entry (full verbatim) archived to [`docs/phase_marker_history.md`](docs/phase_marker_history.md) per Phase Marker compactness discipline._
