...
- **Current phase:** Phase 2C post-PHASE2C_8.1 multi-regime evaluation gate (extended) closeout — PHASE2C_8.1 shipped 2026-04-28 evaluating the same 198-candidate batch against four regimes (two PHASE2C_7.1-inherited fully-out-of-sample: bear_2022 + validation_2024; two PHASE2C_8.1-novel train-overlap: eval_2020_v1 + eval_2021_v1) under uniform calibration. Three load-bearing findings: (1) population-level 21-vs-8 in-sample-caveat asymmetry (21 candidates pass both train-overlap regimes jointly vs 8 pass both fully-out-of-sample regimes jointly; direction matches Concern A spec §7.4 + §10.6 prediction); (2) cohort_a_filtered = 0 with lone unfiltered cross-regime survivor `0845d1d7898412f2` (theme `volume_divergence`; `volume_surge_momentum_entry`) excluded from filtered cohort by single-trade margin in bear_2022 (19 trades vs ≥20 threshold); (3) audit-only partition origin (wf_test_period_sharpe = -0.072 < 0.5 primary threshold) reinforces PHASE2C_7.1 audit-only-mirror finding direction at n=4. Three explicit bounds: no DSR/PBO/CPCV statistical-significance claim; no deployability claim; no mechanism proof. Verification chain at three independent layers (production / Codex / permanent in-repo recompute gate); 1400/1400 tests green at commit `8086adf`. Closeout at [`docs/closeout/PHASE2C_8_1_RESULTS.md`](docs/closeout/PHASE2C_8_1_RESULTS.md); ten tracked-fix register entries (Q-S4-5/7/9/10/11/12/13/14/15/16) with four methodology-codification candidates clustered at §10.2.
- **Completed:**
