...
- **Current phase:** Phase 2C post-PHASE2C_7.1 multi-regime decision point — PHASE2C_7.1 multi-regime evaluation gate shipped 2026-04-27 with a load-bearing finding that selection power is regime-dependent and not robustly preserved across the two tested regimes (`v2.regime_holdout` = bear_2022; `v2.validation` = validation_2024). Three §5 cuts: within-2024 inversion (primary 22/44 = 50.0% > audit-only 65/154 = 42.2%; opposite of PHASE2C_6's 2022 direction), cross-regime intersection mirror (audit-only 8/154 = 5.2% > primary 0/44 = 0.0%; matches 2022 anti-selection direction), PHASE2C_6 carry-forward asymmetry (8/12 = 66.7% audit-only carried; 0/1 primary). Three cuts are different estimands; mechanism adjudication remains undetermined. Follow-up scoping questions enumerated in [`docs/closeout/PHASE2C_7_1_RESULTS.md`](docs/closeout/PHASE2C_7_1_RESULTS.md) §9 (Q-B1 additional regimes, Q-B2 calibration variation, Q-B3 cohort deep-dive, Q-B4 DSR infrastructure).
- **Completed:**
...
- **Current batch_id:** `b6fcbf86-4d57-4d1f-ae41-1778296b1ae9` (Phase 2C Phase 1 walk-forward; corrected-engine re-run in `_corrected/` directory is canonical; same batch consumed by PHASE2C_6 single-regime evaluation runs at `data/phase2c_evaluation_gate/{smoke,primary,audit}_v1/` and by PHASE2C_7.1 multi-regime evaluation runs at `data/phase2c_evaluation_gate/{audit_2024_v1, audit_2024_v1_filtered, comparison_2022_vs_2024_v1}/`)
- **Current UTC-month spend (April 2026):** ~$8.65 (D7 Stage 2d $5.89 + Phase 2C Batch-1 $2.30 + smoke and dry-run batches; per `agents/spend_ledger.db` `ledger` table, queried 2026-04-26; PHASE2C_6 + PHASE2C_7.1 evaluation work incurred no API spend — local backtest evaluation only)
