...
- **B-iii sub-cycle (2026-06-02, WITHIN the freeze — does NOT un-freeze):** first exploration of freeze **deferred-open path (ii)** [a pre-registered Bayesian / continuous-evidence evaluation layer]. Charlie-registered pure-analytical **SCOPING** → pre-registered spec (mechanisms M1–M7 + 2-axis §5 rule + 2-prong anti-motivated clause; two-pass design+written-spec B2) → analysis (SV1–SV7 read-only, `backtest/eval_layer_scoping.py`, 18 tests). **VERDICT: CONDITIONAL-BUILD (Charlie-accepted binding read).** M3 (cross-cycle pooling) **backfires** [posterior −0.26/−1.60 ≪ deploy line] + M1 (posterior-sizing) capital-dangerous [P(SR≤0)=21%] = killed; M2/M4/M5 add only time/honesty (M5 e-value needs ~2.66 yr); **M6 (change-the-estimand) plausibly RAISES confirmation power at BTC heavy tails** — the sign/median test gains efficiency over Sharpe under heavy tails (ARE=(2·f(0))² crosses 1 at γ4≈11.84; the strategy anchor γ4≈11.3 is just below [ARE 0.994], overtaking at the higher kurtosis of raw hourly returns), BUT a directional power gain confirms a MEDIAN shift not a deployable SHARPE edge (skew/cost) → §5 CONDITIONAL branch (i). **Recommended estimand-deployability study ENDORSED but NOT authorized** (separate future register; anti-pre-emption — endorsed ≠ authorized). **Result-B2 caught a verdict-FLIPPING SV6 bug** (first pass DON'T-BUILD on a Gaussian-f(0) sign-test error; Codex independent recompute caught it → re-derived + re-B2'd → CONDITIONAL); SEAL-eve 2-leg run. **Deferred-open path (ii) is thus NOT closed** (live M6 lead). Sealed `tier6_dsr_v1/` 4/4 byte-unchanged; local-only. Branch `bayesian-eval-layer-scoping` (off `main`) **→ merged to `main`** (fast-forward, this SEAL); commits `0075c744`(spec)→[this SEAL]; METHODOLOGY_NOTES **§41** (4 lessons: reason-then-compute can flip the sign; independent-recompute is the B2's load-bearing value on a number; statistical-power≠deployment-relevance; a pinned criterion forces verdict-change to rest on substance). Memo [`docs/phase5/BAYESIAN_EVAL_LAYER_SCOPING.md`](docs/phase5/BAYESIAN_EVAL_LAYER_SCOPING.md). Arc tag `bayesian-eval-layer-scoping-v1` (proposed).

- **Estimand-deployability confirmation sub-cycle (2026-06-02, WITHIN the freeze — CLOSES the B-iii M6 lead):** Charlie registered the B-iii-endorsed estimand-deployability study; a design-point 2-leg converged near-foreseeable DON'T-BUILD → Charlie registered a **minimal-confirmation run** of 4 pillars (`backtest/eval_estimand_deploy.py`, 8 tests, read-only). **VERDICT: DON'T-BUILD (Charlie-accepted binding read), no surprise.** P1 skew-divorce (directional gate fires P(up)=0.566 at Sharpe −0.02 → advantage in the NON-deployable zone); P2/P3 deployable-band net benefit after 2N\* multiplicity = **+2.08pp on-grid / 7.09pp global out-of-grid** (dense 3960-cell box → no hidden peak) ≪ the 10pp tripwire; P4 runs-test = confirmation-layer category error (a serial edge shows in strategy P&L the gate scores). **Closes the M6 lead → the continuous-evidence / Bayesian / alternative-estimand evaluation-layer family is EXHAUSTED as a build-now confirmation-power lever** (reinforcing the confirmation-power freeze; the directional estimand survives only as an honesty-annotation). Result 2-leg-B2 both SOUND (advisor reversed its over-correction worry on the out-of-grid evidence); SEAL-eve 2-leg. **NOT closed:** the freeze's other deferred-open paths (fresh-OOS accrual; structurally-different frames) untouched; 2024/2025 holdout UNSPENT. Sealed `tier6_dsr_v1/` 4/4 byte-unchanged; local-only. Branch `estimand-deployability-confirm` (off `main`) **→ merged to `main`** (fast-forward, this SEAL); commits `ad21f771`(spec)→[this SEAL]; METHODOLOGY_NOTES **§42** (minimal-confirmation-as-guard; dense out-of-grid global-max forecloses false-negatives). Memo [`docs/phase5/ESTIMAND_DEPLOYABILITY_CONFIRMATION.md`](docs/phase5/ESTIMAND_DEPLOYABILITY_CONFIRMATION.md). Arc tag `estimand-deployability-confirm-v1` (proposed).

**Sealed phase history**: full historical detail at [docs/phase_marker_history.md](docs/phase_marker_history.md). Compact summary of the 8 most recent register-events (estimand-deployability confirmation + B-iii sub-cycle + post-Path-D freeze current + 5 prior):

| Phase / Arc | Seal commit | Closeout / canonical artifact | Tag |
|---|---|---|---|
| **Estimand-deployability confirmation** (within the post-Path-D freeze; CLOSES the B-iii M6 lead) — **VERDICT: DON'T-BUILD (Charlie-accepted 2026-06-02)**, no surprise. Minimal-confirmation run (4 pillars; `backtest/eval_estimand_deploy.py`, 8 tests, read-only) after a design-point 2-leg converged near-foreseeable. P1 skew-divorce (directional fires P(up)=0.566 at Sharpe −0.02 → advantage in NON-deployable zone); P2/P3 deployable-band net benefit after 2N\* = +2.08pp on-grid / 7.09pp global out-of-grid (dense 3960-cell box) ≪ 10pp tripwire; P4 runs-test = confirmation-layer category error (serial edge shows in strategy P&L the gate scores). **Closes M6 → the continuous-evidence / Bayesian / alt-estimand eval-layer family EXHAUSTED as a build-now confirmation-power lever** (reinforces the freeze; directional estimand survives as honesty-annotation only). Result 2-leg-B2 both SOUND (advisor reversed its over-correction worry on the dense out-of-grid evidence); SEAL-eve 2-leg (caught + removed a monotonicity overclaim; sealed the search in the harness). Other deferred-open paths untouched; holdout UNSPENT. Sealed `tier6_dsr_v1/` 4/4 byte-unchanged; local-only. METHODOLOGY_NOTES §42. | branch `estimand-deployability-confirm` (off `main`) **→ merged to `main`** (fast-forward, this SEAL): `ad21f771`(spec)→[this SEAL]; atomic CLAUDE.md + docs/phase_marker_history.md + METHODOLOGY_NOTES §42 | memo `docs/phase5/ESTIMAND_DEPLOYABILITY_CONFIRMATION.md`; spec `docs/superpowers/specs/2026-06-02-estimand-deployability-confirmation-design.md`; harness `backtest/eval_estimand_deploy.py` | `estimand-deployability-confirm-v1` (proposed) |
| **B-iii** continuous-evidence / Bayesian eval-layer **SCOPING** (within the post-Path-D freeze; does NOT un-freeze) — **VERDICT: CONDITIONAL-BUILD (Charlie-accepted 2026-06-02)**. Pure-analytical scoping of freeze deferred-open path (ii): 7 mechanisms M1–M7 + pre-registered 2-axis §5 rule + 2-prong anti-motivated clause (two-pass design+written-spec B2); analysis SV1–SV7 (`backtest/eval_layer_scoping.py`, 18 tests, read-only). M3 cross-cycle pooling **backfires** [posterior −0.26/−1.60] + M1 sizing capital-dangerous [P(SR≤0)=21%] = killed; M2/M4/M5 time/honesty only (M5 e-value ~2.66 yr); **M6 change-the-estimand plausibly RAISES confirmation power at BTC heavy tails** (sign/median ARE=(2·f(0))² crosses 1 at γ4≈11.84; strategy anchor γ4≈11.3 just below [ARE 0.994], overtakes at higher raw-hourly kurtosis), CONDITIONAL-gated by deployment-relevance (directional power ≠ deployable Sharpe; skew/cost). **Recommended estimand-deployability study ENDORSED-not-authorized** (fresh future register; anti-pre-emption). **Result-B2 caught a verdict-FLIPPING SV6 bug** (Gaussian-f(0) sign-test error → DON'T-BUILD; Codex independent recompute → flipped to CONDITIONAL → re-B2'd SOUND-WITH-CAVEATS); SEAL-eve 2-leg. Sealed `tier6_dsr_v1/` 4/4 byte-unchanged; local-only. METHODOLOGY_NOTES §41 (4 lessons). | branch `bayesian-eval-layer-scoping` (off `main`) **→ merged to `main`** (fast-forward, this SEAL): `0075c744`(spec)→[this SEAL]; atomic CLAUDE.md + docs/phase_marker_history.md + METHODOLOGY_NOTES §41 | memo `docs/phase5/BAYESIAN_EVAL_LAYER_SCOPING.md`; spec `docs/superpowers/specs/2026-06-02-bayesian-eval-layer-scoping-design.md`; harness `backtest/eval_layer_scoping.py` | `bayesian-eval-layer-scoping-v1` (proposed) |
