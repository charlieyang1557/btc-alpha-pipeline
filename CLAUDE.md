...
- **Current phase:** R3.1d cost-grid re-anchor cycle (R3.1d = Phase B Pre-Sequence Roadmap V3 Tier 3 cost-grid re-anchor Bucket-1 investigation note per Template B; structural analog to Phase 5.1 + 5.2 + Phase A + R1.2 + R3.1a + R4.1 SEAL cycles; fired by Charlie 2026-05-18 as register-event "1 → 3 → 4 with conservative-anchor synthesis approved, R3.1d fire authorized") **SEALED** at R3.1d V_SEAL register-event boundary (canonical R3.1d artifact at [`docs/phase5/R3_1D_COST_GRID_REANCHOR_NOTE.md`](docs/phase5/R3_1D_COST_GRID_REANCHOR_NOTE.md) sealed at R3.1d V_SEAL bundle commit `44840a3` + this Phase Marker advance commit; R3.1d = 620+ lines / 12 main §§ + cycle metadata header + §12 companion artifact appendix). **Locked sub-decisions (Charlie register chain through V_SEAL 2026-05-19):** SD1 = Option A 15 bps/side (30 bps RT) conservative anchor per Phase 5.2 §6.4 Branch.A "upper end of heuristic range" + Phase 4 PLAN §1.4 dual-source backing; SD2-D sidecar config pattern (NEW `config/execution_phaseb_spot_15bps.yaml` alias; main `execution.yaml` unchanged → preserves post-E.3 config_hash `sha256:db2ce75bd41e8513`; new `cost_anchor_id TEXT` schema column in `experiment_registry.runs` for path-based forensic discrimination per V3-PA replaces non-injective V2-P5 `fee_model_label` mapping); SD3-D tiered Pillar 1 policy (Phase 1-2 archival grandfathered at 7bps; new Tier 5/6 work at 15bps conservative anchor; new CLAUDE.md HARD CONSTRAINT "Conservative-Anchor Gate Integrity"); SD4-A bundle R4.1 hygiene patches (inline-edit class per R3.1d §5.4 precedent; stale-status strings on lines 5/42/462/498 of R4.1 doc updated to post-SEAL state); SD5-B separate register-event for R2.1 audit-criterion pre-commit; SD6-A explicit R3.1b/c non-foreclosure with Phase 4 paper-trading deployment trigger; SD7-A Bucket-1 Template B; SD8-A 2-leg subagent default; SD9 V_SEAL lock — **Tier 5 gate = Option A (`holdout_sharpe > 0` strict at 15 bps/side; effect on PHASE2C_15 cohort_a = 18/39 = 46.15% per V3 §6.7 strict-Sharpe table with verified scipy CIs)** + **Tier 6 gate (R6.1 promotion class) PRE-COMMITTED FWER-style multiplicity correction REQUIRED** — project-architecture rationale: serial individual-strategy capital commitment, no cross-strategy diversification at deployment → FWER framework. Eligible Tier 6 instruments: DSR per Bailey-López de Prado 2014 (preferred); Romano-Wolf stepdown; Westfall-Young permutation FWER; heuristic DSR `sqrt(2*ln(N))` acceptable interim screen only. **BH-FDR NOT eligible** (FDR vs FWER framework distinction — V3.5-PD correction removed earlier V2-P9 fabricated BH-FDR eligibility caught by Codex final pre-SEAL grep on R1.2; R1.2 doc contains ZERO hits for DSR/BH-FDR/FDR/Bonferroni/multiplicity terms — propagated Advisor V1 hallucination). N specification per CLAUDE.md HARD CONSTRAINT (`hypotheses_attempted` not `hypotheses_approved`). Instrument variant + threshold + N value locked at R6.1 V_SEAL. R6.1 cycle REQUIRED for Tier 6 promotion (not eligible-not-named). DSR advisory annotation at Tier 5 = eligible at R5.1 implementation cycle. **Cycle scope:** R3.1d V_SEAL = governance commitment (Tier 5 cost gate + Tier 6 multiplicity gate pre-commit) + new config file + schema migration + CLAUDE.md HARD CONSTRAINT addition + R4.1 hygiene patches. NO numeric change to existing Phase 1-2 backtest results (Pillar 1 archival preserved; new work at 15bps anchor). NO API spend (cycle = local-only). NO new data acquired. **10 Charlie register fires across R3.1d cycle through V_SEAL:** (1) cycle entry "1→3→4 with conservative-anchor synthesis approved, R3.1d fire authorized"; (2) sub-decision Round 1 reviewer dispatch; (3) PFR cross-validation dispatch; (4) "ratify all" SD1-SD9 + P1-P7 lock; (5) "fire V1 draft + dispatch reviewer round"; (6) V1 round Codex stall recovery "cancel and kill, then rerun blind lean for codex and advisor review freshly. so we can verify the hallucination issue"; (7) "Apply V2 patches + 进入 V2 reviewer round + Memory codification investigation"; (8) V2 → V3 ratify + PFR rule-Y dispatch; (9) "dispatch to reviewer round for a final review before fire seal" + manual advisor agent model upgrade sonnet→opus; (10) "接受 6 个 V3.5 patches + 接受最终 V_SEAL register text + Fire V_SEAL register". **Adjudication register at V_SEAL:** 8 reviewer dispatches (sub-decision Round 1 + PFR + V1 original killed-at-stall + V1 fresh redispatch + V2 + V3 PFR + final pre-SEAL × 2 legs each = ~14 effective dispatches with stall recovery). **Codex reliability empirically excellent this cycle:** 0/5 completed-dispatch hallucinations + 1 stall (recovered cancel+kill at V1 original); caught V2 BLOCKING (slippage.py:11 fee_model_label injectivity defect) + propagated Advisor hallucination at final pre-SEAL gate (R1.2 §6 BH-FDR fabrication via grep). **Advisor reliability degraded this cycle:** 15+ verified hallucinations across 7 dispatches (~85% per-dispatch rate; vs cumulative baseline ~22%); Mode A structural (file:line/exact-quote/numerical-content + wholesale document fabrication at V3 PFR); mandatory `[VERIFIED via grep]` token in dispatch briefs did NOT eliminate failures (V1 fresh redispatch diagnostic confirmed Mode A is structural not prompt-fixable); model upgrade sonnet→opus at decision-point round 6 yielded substantially improved performance (honest uncertainty disclosure + judgment-quality methodology output). **B2 standing rule LOCKED 2026-05-19** per `feedback_reviewer_routing_subagent_default.md` Reading 3 pilot CLOSED — Codex cross-model leg is load-bearing for SEAL-class canonical artifacts. **Mode A task-class severity refinement codified** (V3.5-PI): Advisor hallucinations severity scales with citation-density of task class — CRITICAL for artifact verification (~85% rate, content unusable); MANAGEABLE for substantive methodology judgment (~1 per dispatch, content recoverable). Operational refinement REVISED 2026-05-19 post-V_SEAL per Charlie register: V3.5-PI "skip Advisor on citation-dense" rule REVERTED — Advisor (opus) INCLUDED across all task classes for cross-model validation pilot under new opus regime; Mode A independent re-verification preserved as safety overlay; re-evaluate after 3-5 opus citation-dense dispatches per `feedback_reviewer_routing_subagent_default.md` REVISED section. **Option 1A atomicity binding empirical 11th trigger:** this Phase Marker advance commit atomically updates `docs/phase_marker_history.md` per Option 1A binding (`feedback_claude_md_freshness.md`); 11th trigger — cross-cycle robustness now 11-instance validated. **Forward-only carry-forward observations from R3.1d V_SEAL (4 new):** (1) Mode A task-class severity refinement codified; (2) Propagated-hallucination text in V2-P9 caught by Codex final pre-SEAL grep — demonstrates cross-model leg is structurally required for SEAL-class artifacts; (3) Advisor opus upgrade yielded substantial decision-point quality improvement (anti-self-serving discipline + honest uncertainty + Romano-Wolf/Westfall-Young surfaced as alternative FWER instruments); (4) R1.2 §6 contains NO multiplicity-correction prose despite Advisor V1 hallucination citing it — this was a propagated fabrication, not an R1.2 errata trigger. **Active next action:** R3.1d SEAL bundle pushed (2 commits: SEAL artifacts commit `44840a3` + this Phase Marker advance commit `<TBD>` with atomic history file update); R3.1b/c eligible-not-named when Phase 4 paper-trading infrastructure deploys; Phase B Pre-Sequence Roadmap V3 Tier 2 conditional prereqs (R2.1 / R2.2 / R2.3) + R5.1 candidate-subset under SPOT venue + R5.2 selection-inflation handling + R6.1 promotion class (REQUIRED per SD9 lock; instrument + threshold + N at R6.1 V_SEAL) + Tier-0 pause + Phase 2.5 bandit-dedup (parked) + pre-existing noise cleanup (.DS_Store + docs/d7_stage2c/* 13+ session carry-forward) + memory codification follow-up + advisor opus model effects pilot (extended observation) + other Charlie-specified — all eligible at separate Charlie register-event boundary per anti-pre-emption + Phase 5.1/5.2/Phase A/R1.2/R3.1a/R4.1 SEAL precedent. NO tag per Bucket-1 investigation note ≠ arc-level closeout. _R4.1 + R3.1a E.3 errata cycle entry detail (full verbatim) preserved at [`docs/phase_marker_history.md`](docs/phase_marker_history.md) per sealed-content invariance + Phase Marker compactness discipline._
...
|---|---|---|---|
| R3.1d cost-grid re-anchor cycle SEAL | `44840a3` (+ Phase Marker advance — this commit) | [`docs/phase5/R3_1D_COST_GRID_REANCHOR_NOTE.md`](docs/phase5/R3_1D_COST_GRID_REANCHOR_NOTE.md) + [`config/execution_phaseb_spot_15bps.yaml`](config/execution_phaseb_spot_15bps.yaml) + R4.1 hygiene patches + CLAUDE.md HARD CONSTRAINT (Conservative-Anchor Gate Integrity) + schema migration (`cost_anchor_id TEXT`) | — |
