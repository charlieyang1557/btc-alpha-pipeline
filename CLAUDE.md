...
- **Current phase:** B-C-narrow data-recovery cycle (driver: R6.1 V_SEAL §10 binding precondition — recover per-bar return series + per-candidate γ3/γ4 moments + registry linkage for the `phase4_forward_2026_15bps_v1` cohort_a 39-candidate cohort, to unblock post-V_SEAL Tier 6 evaluation application per R6.1 Path α invariant; architecture Approach D' producer-edit + minimum engine extension; PV3-SPLIT-BY-PHASE per-phase PFR+SEAL cadence; cycle entry fired by Charlie 2026-05-26 as register N1) **SEALED** at B-C-narrow data-recovery cycle SEAL register-event boundary 2026-05-29 (canonical artifact at [`docs/phase5/B_C_NARROW_DATA_RECOVERY_NOTE.md`](docs/phase5/B_C_NARROW_DATA_RECOVERY_NOTE.md) v2; arc-level closeout; arc tag `b-c-narrow-data-recovery-v1`). **Cycle structure:** 5 phases — Phase 0 engine-extension SEALED `f112599` (RegimeHoldoutResult.equity_curve + 4 LC-b kwargs + atomic write-then-registry sequencing) + Phase 1 pre-impl gates SEALED `b10ffb2` (G1 engine-diff audit + G2 DSL backward-compat + G3 raw_payloads + G3.5 smoke, all PASS) + Phase 2 producer TDD SEALED `d42beeb` (`--enable-b-c-narrow-recovery` wiring; R9 split PRE-flight/POST-fire; W0 identity + W3 archive + W4 finalize helpers) + Phase 3 fire/data-recovery SEALED `9d54b6b` (T13 fire + V4 gate 12/12 ε=1e-6 + relocated full-suite 2372 passed / 2 xfailed). **Phase 3 first-fire→revert→re-fire arc:** first fire ran GREEN but the Step-14.5 full-suite gate surfaced 3 sealed-plan defects (Gap 1 canonical git-tracked-not-gitignored + Gap 2 pc9 baseline + Gap 2b Step-13.3 isolation); reverted to green SEAL `b96d3a8` per Charlie Option A; v9 re-fire errata + B2 re-review + v9.1 polish + Rule-2 SEAL-eve CLEAR → re-SEAL `38e1291`; re-fire executed clean per corrected §V9.4 (Q1=a/Q2=a/Option A adjudications). **Deliverable:** 39 candidates recovered (20 pass / 19 fail / 0 holdout_error, deterministic across both fire attempts); per-bar return series (39 returns_per_bar.parquet) + γ3/γ4 + T_obs + registry linkage (1 parent batch_summary + 39 child regime_holdout rows) committed `ff0c576`; pre-recovery original preserved byte-identical in committed archive `6d4bb7f` + git `7c8f4a7`. **B2 standing rule LOCKED 2026-05-19 + 3-layer safety architecture** reaffirmed at cumulative scale; **Rule 2 SEAL-eve OPERATIONALLY REQUIRED re-vindicated** (Phase 3 v8 SEAL-eve R1 Codex empirical run caught 2 architectural BLOCKINGs that 5 PFR rounds + Advisor static read missed; Layer-3 orchestrator caught 1 Codex hallucination at Phase-4 NOTE review → PUSHBACK; Codex result-delivery stall once at Phase-4 review resolved at redispatch). **Option 1A atomicity binding empirical 20th trigger:** this Phase Marker advance commit atomically updates `docs/phase_marker_history.md` per Option 1A binding; 20th cumulative trigger. **Active next action:** **post-V_SEAL Tier 6 evaluation application** NOW UNBLOCKED (B-C-narrow binding precondition satisfied; eligible-not-named under R6.1 Path α invariant; R6.1 methodology + §12 errata supersession layer applied to the recovered 39-candidate cohort) + **§36 METHODOLOGY_NOTES codification batch** from this cycle (NAME-only per NOTE §9: line-citation-drift→quoted-text-anchor invariant + collection-time-crash discovery + 3-leg Q2 adjudication + Codex-hallucination-caught-by-Layer-3 + Rule-2 SEAL-eve re-vindication + execution-time-discovery→revert→patch pattern; eligible at separate codification register-event) + all prior carry-forward backlog (B-C-extended SEAL bundle components 2 §35-codification + 3 memory-codification remaining; R6.1-A/B/C codification; R5.2 memory codification; conditional RW/WY framework reopen; SD-E-γ stationary bootstrap upgrade; Form A/B framing tension resolution; R2.2; P2a; R3.1b/c; supplementary IS-OOS; Phase 4 paper-trading deployment; Bonferroni eligibility re-evaluation; Tier-0 pause; Phase 2.5 bandit-dedup parked; pre-existing noise cleanup; advisor opus model effects pilot; project pause / strategic-absorption; other Charlie-specified) — all eligible at separate Charlie register-event boundary per anti-pre-emption + R6.1/R5.2/R5.1/R2.3/§34/R2.1/R2.0/R3.1d/R4.1/Phase 5.1/5.2/Phase A/R1.2/R3.1a/B-C-extended SEAL precedent. **Arc tag `b-c-narrow-data-recovery-v1`** per arc-level closeout class (precedent `phase4-forward-test-v1` + `phase5-diagnostic-execution-v1`). _R6.1 V_SEAL §2.2 narrow patch errata SEAL Phase Marker entry (full verbatim) archived at [`docs/phase_marker_history.md`](docs/phase_marker_history.md) per sealed-content invariance + Option 1A atomic binding + Phase Marker compactness discipline._
