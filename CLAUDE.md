...
- **Current phase:** Phase 2C post-PHASE2C_6 evaluation-gate decision point — PHASE2C_6 single-regime evaluation gate shipped 2026-04-27 with a load-bearing negative selection-power finding (in this batch, against 2022 regime, the corrected WF gate did not enrich for holdout survival; audit-only candidates survived at 7.79% vs primary at 2.27%). PHASE2C_7+ scoping decision pending; open questions enumerated in [`docs/closeout/PHASE2C_6_EVALUATION_GATE_RESULTS.md`](docs/closeout/PHASE2C_6_EVALUATION_GATE_RESULTS.md) §9: AND-gate empirically underdetermined (n=1), mechanism distinction (regime-mismatch vs pattern-overfit) requires multi-regime evidence, audit-only survivors as diagnostic material not strategy recommendations, trade-count threshold question. Closeout explicitly does not pre-commit forward decisions; PHASE2C_7+ scoping is the appropriate venue.
- **Completed:**
...
- **Current batch_id:** `b6fcbf86-4d57-4d1f-ae41-1778296b1ae9` (Phase 2C Phase 1 walk-forward; corrected-engine re-run in `_corrected/` directory is canonical; same batch consumed by PHASE2C_6 evaluation-gate runs at `data/phase2c_evaluation_gate/{smoke,primary,audit}_v1/`)
- **Current UTC-month spend (April 2026):** ~$8.65 (D7 Stage 2d $5.89 + Phase 2C Batch-1 $2.30 + smoke and dry-run batches; per `agents/spend_ledger.db` `ledger` table, queried 2026-04-26; PHASE2C_6 evaluation work incurred no API spend — local backtest evaluation only)
