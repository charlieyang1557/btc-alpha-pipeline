...
- **Current phase:** Path B mechanism-first OHLCV re-mine — **HARNESS ARC (Sections A–E, Tasks 1–30) BUILT + cycle-SEAL B2-reviewed + COMMITTED** on feature branch `pathb-mechanism-first-rethink` (7 commits ahead of `main`@`96c7370`; **NOT merged, NOT pushed** — `superpowers:finishing-a-development-branch` merge/PR decision pending per Charlie register), 2026-05-30 UTC. **This is the Charlie-registered bounded one-cycle alpha-source rethink** (the A-1 SEAL's "Active next action"): re-mine EXISTING OHLCV process-first (cost-aware objective + min-trade/occupancy floor + ternary sizing + single-asset WorldQuant-101 ops) across 3 pre-registered mechanism-first hypotheses (**H1** `intrabar_push_fade` / **H2** `vol_regime_switch` / **H3** `decay_trend_persistence`), **N\*=3** (Step −1 LOCK, minimal grid), DSR-FWER at the 15bps spot anchor. **Commits:** `504f957` design spec v2.1 + 30-task plan · `aa1465e` Step −1 pre-registration LOCK · `7d37409` A (leakage guards G1–G4a) · `22e56da` B (decay_linear primitive + 5 factors, registry 18→23) · `2c2577c` C (ternary SizingSpec node) · `cbc0586` D (N\* plumbing + sealed-dir HARD guard) · `cd88020` E (evaluation harness + earned-negative taxonomy). **CRITICAL framing — NO VERDICT YET:** Section E builds the evaluation HARNESS + earned-negative taxonomy LOGIC; it does NOT run the experiment. **The actual mechanism-first re-mine RUN (execute H1/H2/H3 on real data → real per-variant Tier-5 `holdout_sharpe` + Path-B `CandidateMoments` → feed the harness → earned-negative-or-B-positive verdict) is a DISTINCT Charlie-registered operational step, NOT one of the 30 tasks.** **Sealed-artifact invariant held throughout:** `tier6_dsr_v1/` + the `phase4_forward_2026_15bps_v1` cohort byte-untouched (sha256 4/4 baseline-match; two-layer sealed-dir guard — `main()` consumer + `evaluate_cohort()` producer); `tier6_dsr.py` N\* plumbing is default-path byte-invariant (Task 22 byte-regression) + `Z_PASS` frozen for scipy-version-independent reproduction. **B2 2-leg at the Section D boundary + the comprehensive cycle-SEAL B2** (Codex `codex:codex-rescue` + `quant-research-advisor`): Codex caught a real producer-API guard-bypass at D (fixed); the cycle-SEAL B2 caught **2 spec/LOCK-vs-code contradictions plan-transcription carried in** — `PATHB_N_STAR` 18→**3** (binding Step −1 LOCK; 18 was the dead cohort's N\*) + H3 driver `volume_zscore_24h`→`decay_linear_close_48 > _168` (design spec), both **advisor-found + corrected**; Codex CRITICAL "evaluate_cohort token in pathb_dsr_fwer" REJECTED as a verified false-positive (docstring prose only, no live import/call). Full suite **2698 passed / 8 skipped / 2 xfailed**; pc9 baseline 2360→2582 (+222 across A–E). Local-only (no project Anthropic-API spend). **Option 1A atomicity binding 23rd trigger:** this Phase Marker advance commit atomically updates `docs/phase_marker_history.md`. **Active next action(s):** (i) `finishing-a-development-branch` — Charlie's merge/PR/keep-branch decision (branch unpushed per Charlie register); (ii) the verdict-producing experimental RUN (own register-event, anti-pre-emption); (iii) Path A (crypto-native data — funding/OI/basis/liquidations) remains the CONDITIONAL successor, escalated only on a process-refuted earned-negative per the §9 advisory; (iv) prior carry-forward backlog (A-1's §36 METHODOLOGY_NOTES codification incl. this arc's advisor spec-contradiction catches + the advisor-hallucination-vs-live-verify lesson; conditional A-2/RW-WY; B-C/R-series codification; Phase 4 paper-trading; Phase 2.5 bandit-dedup parked; remaining-797 abspath sanitize; pre-existing noise cleanup; other Charlie-specified) — all eligible at separate Charlie register-event boundaries per anti-pre-emption + SEAL precedent. **Arc tag `pathb-mechanism-first-harness-v1`** (proposed; harness-arc closeout class — a future verdict-run would carry a separate tag). _A-1 SD-E-γ stationary-bootstrap suitability cycle SEAL Phase Marker entry (full verbatim) archived at [`docs/phase_marker_history.md`](docs/phase_marker_history.md) per sealed-content invariance + Option 1A atomic binding + Phase Marker compactness discipline._
...
|---|---|---|---|
| Path B mechanism-first OHLCV re-mine — HARNESS ARC (Sections A–E, Tasks 1–30) BUILT + cycle-SEAL B2-reviewed + COMMITTED on branch `pathb-mechanism-first-rethink` (NOT merged/pushed; `finishing-a-development-branch` pending); Charlie-registered bounded one-cycle alpha-source rethink — re-mine EXISTING OHLCV process-first (cost-aware + min-trade/occupancy floor + ternary sizing + WQ-101 ops; H1 `intrabar_push_fade` / H2 `vol_regime_switch` / H3 `decay_trend_persistence`; **N\*=3** Step −1 LOCK; DSR-FWER 15bps). **NO VERDICT YET** — Section E builds the HARNESS + earned-negative taxonomy LOGIC; the verdict-producing experimental RUN is a distinct Charlie-registered step. Sealed `tier6_dsr_v1/` + cohort byte-untouched (sha256 4/4); two-layer sealed-dir guard; default-path N\* byte-invariant. Cycle-SEAL B2 caught 2 spec/LOCK-vs-code contradictions (PATHB_N\* 18→3 per LOCK; H3 driver volume→decay-trend per spec — both advisor-found+corrected) + Codex producer-guard (D, fixed) + Codex CRITICAL false-positive rejected. Full suite 2698 passed; pc9 2360→2582. | branch `pathb-mechanism-first-rethink` 7 ahead of `main`@`96c7370` (unpushed): `504f957`/`aa1465e`/`7d37409`/`22e56da`/`2c2577c`/`cbc0586`/`cd88020`; Phase Marker advance this commit (atomic CLAUDE.md + docs/phase_marker_history.md per Option 1A 23rd trigger) | [`docs/superpowers/specs/2026-05-30-pathb-mechanism-first-rethink-design.md`](docs/superpowers/specs/2026-05-30-pathb-mechanism-first-rethink-design.md) (v2.1) + 30-task plan + Step −1 LOCK; harness `backtest/pathb_*.py` + `scripts/pathb_*.py` | `pathb-mechanism-first-harness-v1` (proposed) |
