...
- **Current phase:** Path C perp-spot **basis** axis mechanism-first mine — **VERDICT: `process_refuted_for_this_grid` (earned negative); next-axis escalation AUTHORIZED (Charlie 2026-06-01)** — on branch `pathc-basis-scoping` (~33 ahead of `main`, **UNPUSHED**; `finishing-a-development-branch` pending), 2026-06-01 UTC. The Charlie-registered cycle (Path A's escalation-warranted successor) is **COMPLETE**, honestly reframed (§1) as a **higher-frequency robustness re-test of the funding/basis premium signal** (basis ≈ funding by construction; NOT a fresh independent axis): basis-only (Q1); structural frame (b) strict-Path-A-parallel + §37.3 denser-delta resolved to **Option A** (band-widening can't clear the state-class `zero_fraction` floor — the inherited price-trend AND-confirm binds; H2/H3 pre-registered expected-INDETERMINATE); N\*=3; Tier-5 gate forward_2026; all 3 long/flat; H1 NO time-stop (Amendment-A1 inherited) + H3 strict-`<` exact-partition (Codex 2e); Step −1 LOCK + **Ratification Clarification R1** (the 3 pre-data diagnostic tolerances `UNDER_DETERMINED_TRADE_THRESHOLD=10` / `D2_AGREES_TOLERANCE=0.10` / `D1_NONINERT_THRESHOLD=0.10` + D2 same-index correspondence + F3 design — Charlie-ratified pre-run, anti-hindsight; labeling-only, never promotion). **Build:** Phase A markprice ingestion (Binance Vision mark+index 1h klines → `data/raw/btcusdt_markprice_1h.parquet`, 55080 rows 2020-01..2026-04; **real-run header-parse bug found+fixed** — newer Binance Vision kline CSVs carry a header row, the headerless-assuming parser silently dropped ~4y until A6; fixed via header-autodetect mirroring `bulk_download.py`); Phase B native-1h `basis_rel` derivation (`(mark−spot)/spot`, **inner-join-and-log cross-stream guard** — A6 fold, ~1.4% legit non-shared < 5% tol; NO cross-cadence carry, the §37.2 simplification) + 5 basis factors; Phase C H1/H2/H3 DSL builders + `pathc_*` verdict harness + the **dual-orthogonalization D1(vs-momentum)/D2(vs-funding)** diagnostic. **forward_2026 RUN** (2026-01-01..2026-04-16, 2528 bars, 15bps spot, frozen θ=0.90): H1 basis_extreme_fade **−0.87** (19 trades, weak-sane), H2 basis_regime_gate **−2.83** (55, strong-sane, zero_fraction INDETERMINATE as pre-registered), H3 basis_momentum_continuation **0.0** (0 trades — **DEGENERATE**, gate never fired fwd, strong-sane in train). **0/3 Tier-5; 0/3 DSR pass_B** (deflated_z −1.32/−2.34; H3 degenerate excluded from cohort, **N\*=3 unchanged**). any_mechanism_sane=True → process-refuted (not power-limited; no under-determined legs). **Dual-orthogonalization (the cycle's core; fenced):** H2 **VACUOUS** (basis≈funding AND both inert → the cleanest cross-frequency read: 1h basis adds nothing over 8h funding, neither over the dead price leg); H1/H3 `basis_adds_signal` are **ARTIFACTS** (H1 less-bad-losing −0.87 vs −1.62; H3 0-trade-flat beats a down-market baseline), **NOT edges** (0/3 Tier-5 confirms). **Redundancy plausible/consistent, NOT conjunction-confirmed** (advisor caveat — H2's agreement came via an inert D1 → vacuous; the localization rests on the measured losses + mechanism-sanity, §37.3/§9). **Localization:** "the funding/basis premium adds no RESCUE at EITHER 8h or 1h frequency under this grid" — **cross-frequency, NOT family-level** (OI, the genuinely-independent member, untested; basis-not-exonerated; short legs / basis-scaled sizing untried). Magnitude modest (deflated_z ~−1.3/−2.3, Path-A-like ~−2, not Path B's −8). **Disclosed mid-run instrument event:** the authorized fire #1 crashed on a degenerate/flat forward equity (the 0-trade H3) in `CandidateMoments` — pure instrument repair (handle degenerate gracefully = Tier-5 non-pass + excluded from DSR + recorded; `fb805d1`, LOCK untouched), 2-leg-B2'd, then re-run clean. Partial fwd values logged pre-fix = the AUTHORIZED verdict computation (Charlie fired), NOT a no-peek violation. Every phase + a pre-fire **PFR** + the verdict 2-leg-B2'd; the PFR/B2 routine caught real issues at every gate (the input_source contract-widen, the H1-time-stop parity bug, the run-wiring gaps θ-resolution/dual-diagnostic/F3-headline, the H2 de-risk-occupancy wrong-quantity, the degenerate-equity crash); verdict result B2: **Codex COMPUTATIONALLY SOUND + advisor SOUND-TO-SEAL**. **Commits** `1af19f5`(spec)→`<seal>`(this); sealed `tier6_dsr_v1/` sha256 4/4 byte-unchanged throughout (incl. both real runs); canonical verdict artifact `data/phase2c_evaluation_gate/pathc_verdict_v1/pathc_verdict_advisory.json`. pc9 baseline 2780→3014. Local-only (no project Anthropic-API spend). METHODOLOGY_NOTES §38 codifies this arc's lessons. **升级 AUTHORIZED:** **open interest (OI)** is the Charlie-authorized (2026-06-01) next-axis escalation (warranted→authorized) — but a **FRESH scoping cycle** (its own spec → Step −1 LOCK → plan, exactly as Path C was; anti-pre-emption WITHIN the authorization: authorized-to-scope ≠ pre-decided mechanisms). **Active next:** (i) `superpowers:finishing-a-development-branch` — Charlie's merge/PR/keep decision (~33 commits, unpushed; **public repo → pre-push secret/abspath grep first**); (ii) the **OI axis scoping cycle** (AUTHORIZED — the headline next register); (iii) prior carry-forward backlog (push `main` [Path A still local? — see history], Phase 4 paper-trading; Phase 2.5 bandit-dedup parked; abspath sanitize; pre-existing noise) — separate register-events. **Arc tag `pathc-basis-mine-verdict-v1`** (proposed). _Prior (Path A) Phase Marker entry (full verbatim) archived to [`docs/phase_marker_history.md`](docs/phase_marker_history.md) per Phase Marker compactness discipline._

**Sealed phase history**: full historical detail at [docs/phase_marker_history.md](docs/phase_marker_history.md). Compact summary of the 6 most recent register-events (Path C current + 5 prior):

| Phase / Arc | Seal commit | Closeout / canonical artifact | Tag |
|---|---|---|---|
| Path C perp-spot **basis** axis mechanism-first mine — **VERDICT: `process_refuted_for_this_grid` (earned negative); next-axis (OI) escalation AUTHORIZED (Charlie 2026-06-01)** on branch `pathc-basis-scoping` (~33 ahead of `main`, unpushed; `finishing-a-development-branch` pending). Honestly reframed as a **higher-frequency robustness re-test of the funding/basis premium** (basis ≈ funding by construction). Cycle: basis-only (Q1); frame (b)+§37.3→**Option A** (band can't clear `zero_fraction` floor → H2/H3 pre-registered expected-INDETERMINATE); N\*=3; forward_2026; all long/flat; H1 no-time-stop (Amendment-A1 inherited) + H3 strict-`<` partition; Step −1 LOCK + **R1 ratification** (3 pre-data tolerances `=10`/`0.10`/`0.10` + D2 same-index + F3 design, pre-run anti-hindsight, labeling-only). Build: markprice mark+index 1h ingestion (`data/raw/btcusdt_markprice_1h.parquet`, 55080 rows 2020-01..2026-04; **real-run header-parse bug fixed** — Binance added headers to newer kline CSVs) + native-1h `basis_rel` derivation (inner-join-and-log cross-stream guard, ~1.4%<5%; NO carry, §37.2 simplification) + 5 basis factors + H1/H2/H3 DSL builders + `pathc_*` harness + **dual-orthogonalization D1/D2**. **forward_2026 RUN** (2528 bars, 15bps, frozen θ=0.90): H1 **−0.87** (19, weak-sane), H2 **−2.83** (55, strong-sane), H3 **0.0** (0 trades — **DEGENERATE**, gate never fired fwd, strong-sane train); **0/3 Tier-5; 0/3 DSR pass_B** (deflated_z −1.32/−2.34; H3 degenerate excluded, N\*=3 unchanged); any_mechanism_sane → process-refuted. **Dual-ortho (fenced, the cycle core):** H2 **VACUOUS** (basis≈funding AND both inert — cleanest cross-frequency read); H1/H3 `basis_adds_signal` = ARTIFACTS (less-bad-losing / 0-trade-flat-beats-down-market), NOT edges. **Redundancy plausible/consistent NOT conjunction-confirmed** (advisor caveat; localization rests on measured losses + mechanism-sanity §37.3/§9). **Localization:** funding/basis premium adds no rescue at EITHER 8h or 1h — **cross-frequency, NOT family-level** (OI untested). Magnitude ~−1.3/−2.3 (Path-A-like). Disclosed mid-run instrument event: degenerate/flat forward equity crashed CandidateMoments (0-trade H3) → pure instrument repair (`fb805d1`, LOCK untouched) + 2-leg-B2 + re-run clean; partial pre-fix values = the AUTHORIZED computation (not a no-peek violation). Every phase + a pre-fire PFR + the verdict 2-leg-B2'd (caught: input_source widen, H1-time-stop parity, run-wiring gaps, H2 de-risk-occupancy wrong-quantity, degenerate crash); verdict B2 Codex COMPUTATIONALLY SOUND + advisor SOUND-TO-SEAL. Sealed `tier6_dsr_v1/` sha256 4/4 unchanged throughout (incl. both real runs); pc9 2780→3014. **升级 AUTHORIZED:** OI = the next-axis escalation, a FRESH scoping cycle (own spec→LOCK→plan; anti-pre-emption within the authorization). | branch `pathc-basis-scoping` ~33 ahead of `main` (unpushed): `1af19f5`(spec)…`<seal>`(this); Phase Marker advance this commit (atomic CLAUDE.md + docs/phase_marker_history.md + METHODOLOGY_NOTES §38 per Option 1A) | verdict artifact `data/phase2c_evaluation_gate/pathc_verdict_v1/pathc_verdict_advisory.json`; harness `backtest/pathc_*.py` + `scripts/pathc_run_verdict.py`; METHODOLOGY_NOTES §38 | `pathc-basis-mine-verdict-v1` (proposed) |
