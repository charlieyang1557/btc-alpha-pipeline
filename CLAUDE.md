...
- **Current phase:** A-1 SD-E-γ stationary-bootstrap suitability diagnostic cycle (first of the Charlie-registered `A-1 → A-2` methodology-successor sequence, 2026-05-30; R6.1 §6.1-disclosed within-candidate serial-correlation successor) **SEALED** at the A-1 SEAL register-event boundary 2026-05-30 UTC (canonical artifact [`docs/phase5/A1_SERIALCORR_BOOTSTRAP_SUITABILITY_NOTE.md`](docs/phase5/A1_SERIALCORR_BOOTSTRAP_SUITABILITY_NOTE.md); branch `a1-stationary-bootstrap-overlay` merged to `main`; arc tag `a1-serialcorr-suitability-v1`). **Outcome (Path 2 scope-down, Charlie-registered 2026-05-30):** the per-bar-Sharpe stationary-bootstrap (Politis–Romano 1994) measurement of the §6.1 gap is **INFEASIBLE on the sparse `phase4_forward_2026_15bps_v1` cohort** — effective sample = nonzero-bar count 54–758 ≪ T ≈ 2358–2503 (zero_fraction 0.64–0.99); long-block resample degeneracy (skip ≤26% companion / ≤0.62% authoritative) is the acute symptom. **Delivers:** the standing tested primitive (`backtest/tier6_bootstrap.py`, 32 tests) + a **verdict-invariance attestation** (all 18 `excess = SR̂ − SR* < 0` → **0/18, SE-independent**; max_excess −0.0044; tie-back to sealed within machine-ε) + an evidence-grounded suitability diagnostic (`data/phase2c_evaluation_gate/tier6_serialcorr_robustness_v1/`). **NO inflation-ratio measurement** (B2-refuted as sparsity-contaminated). `tier6_dsr.py` + `tier6_dsr_v1/` byte-untouched (sha256 gate + numerical tie-back). **0/18 unaffected.** **B2 2-leg throughout** (spec 16 + plan-PFR 13 + sparse-returns escalation + 2-stage-per-chunk + Rule-2 SEAL-eve, which caught a mis-spliced funnel + a partition span-mislabel + a stale source_commit — all prose/provenance, corrected, no re-fire). Full suite **2484 passed / 2 xfailed**; pc9 baseline 2328→2360 (+32 additive). Local-only (no project Anthropic-API spend). **Strategic pivot (B2-convergent, Charlie-registered):** the project's **binding constraint is the alpha source, not data or methodology** — Tier 6 0/18 + low-trade-frequency sparse returns both indicate the mined DSL strategies lack edge surviving 15bps (best candidate 1.89 z-units short; funnel 993→39→18 makes N\*=18 lenient). **A-2 (RW/WY) + further SD-E-γ-measurement DEFERRED** (rigor on a verdict-invariant negative; load-bearing only for a future near-threshold winner). **Option 1A atomicity binding empirical 22nd trigger:** this Phase Marker advance commit atomically updates `docs/phase_marker_history.md`. **Active next action:** the **Charlie-registered bounded one-cycle alpha-source rethink** (freeze + watchlist the 18; add a crypto-native data family — funding/OI/basis/liquidations; mechanism-first generation with minimum-trade-count + cost-aware objective + relaxed all-in-or-flat sizing; a falsification test earning the right to accept-the-negative) — its own scoping/register cycle (anti-pre-emption) + all prior carry-forward backlog (conditional A-2/RW-WY reopen; §36 METHODOLOGY_NOTES codification incl. this cycle's SEAL-eve catches — mis-spliced-funnel + partition-span-mislabel + stale-source_commit; B-C-narrow/extended + R6.1-A/B/C codification; R5.2 memory codification; R2.2 Monday-pattern mechanism; supplementary IS-OOS; Phase 4 paper-trading deployment; Bonferroni eligibility re-eval; Tier-0 pause; Phase 2.5 bandit-dedup parked; remaining-797 abspath sanitize; pre-existing noise cleanup; other Charlie-specified) — all eligible at separate Charlie register-event boundary per anti-pre-emption + R6.1/R5.2/R5.1/R2.x/R3.x/R4.1/Phase 5.x/B-C-narrow/extended/Tier-6 SEAL precedent. **Arc tag `a1-serialcorr-suitability-v1`** per arc-level closeout class (precedent `tier6-dsr-evaluation-v1` + `b-c-narrow-data-recovery-v1`). _Tier 6 evaluation application cycle SEAL Phase Marker entry (full verbatim) archived at [`docs/phase_marker_history.md`](docs/phase_marker_history.md) per sealed-content invariance + Option 1A atomic binding + Phase Marker compactness discipline._
