...
- **Module / tests / data:** [`backtest/eval_estimand_deploy.py`](../../backtest/eval_estimand_deploy.py) · [`tests/test_eval_estimand_deploy.py`](../../tests/test_eval_estimand_deploy.py) (8 passed) · `data/eval_gate_power_study/estimand_deploy_confirm_v1.json`.
- **Status:** SEALED. **Result 2-leg B2 COMPLETE** (Codex + advisor, both SOUND / SOUND-WITH-CAVEATS, no BLOCK; a dense out-of-grid false-negative search (3960 cells, `out_of_grid_search()`) found the global max net benefit is 7.09pp < the 10pp tripwire → no hidden surprise; the SEAL-eve B2 caught + removed a monotonicity overclaim and sealed the search in the harness; all folded). **Binding read: Charlie ACCEPTED DON'T-BUILD, 2026-06-02.**
...
- **P2 — Deployable-band net benefit ≪ tripwire (confirmed).** The net detection benefit of *adding* a standalone directional gate (FWER-preserved at 2N\*) over the Sharpe gate alone, maximized over the **entire** deployable grid (Sharpe ∈ {1.0,1.25,1.5} × γ4 ∈ {11.3,30,60,100} × N\* ∈ {3,18,39}), is **+2.08pp** — at the *most generous* corner (1.5 Sharpe, γ4=100, N\*=3, independence-assumed OR). That is **≪ the 10pp tripwire** (which is ~9× any achievable gain). The directional advantage is real but trivially small where it is deployable. **Out-of-grid false-negative search (result-B2, advisor; reproduced):** even cranking *every* dial to absurd *non-deployable* extremes (Sharpe 2.0 × γ4 1000 × N\*=1), the **global** max net benefit over a dense out-of-grid box (3960 cells; sealed in `out_of_grid_search()`) is **7.09pp — still < 10pp**, so the coarse deployable grid hides no ≥10pp peak (the quantity is *not* globally monotone — a SEAL-eve B2 caught and removed that overclaim — so this rests on the dense global maximum, not monotonicity). Deployable-constrained (BTC γ4≈11.3, N\*=3) the max is **1.58pp**. No surprise region exists.
...
DON'T-BUILD ⟺ directional advantage in the non-deployable zone (P1 ✓) **AND** deployable-band gain after 2N\* < 10pp (P2 ✓: 2.08pp) **AND** runs-test confirmation-layer-inert (P4 ✓). All three hold → **DON'T-BUILD**. The surprise branch (a deployable region with ≥10pp net gain) did **not** fire. (*Implementation note (result-B2): `run_confirmation()` requires the strict conjunction P1∧P2∧P4 — stricter than the spec §3 "AND/OR" prose; it errs toward `SURPRISE_RECONSIDER` (a real ≥10pp region sets `P2.below_tripwire=False` → reconsider), so the wiring cannot swallow a true surprise — conservative-correct for a DON'T-BUILD.*)
...
- Analysis: 8 tests green; P1 (z=6.6), P3 (+0.218), P4 (P&L Sharpe 0.265) reproduce the design-point previews; P2 (2.08pp max) computed over the full deployable grid. **Result 2-leg B2 COMPLETE** — Codex recomputed all four pillars to 10 digits (SOUND-WITH-CAVEATS); advisor SOUND, reversed its own over-correction worry via the dense out-of-grid search (global max 7.09pp over 3960 cells) + a harder P4 check (sign-persistent runs-test z=−402 → P&L Sharpe +1.03); both confirmed the independence-OR is generous (2.08pp is an upper bound) and the sealed dir byte-unchanged. No BLOCK.
