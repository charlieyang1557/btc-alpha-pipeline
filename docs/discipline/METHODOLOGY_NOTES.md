...
## §38 Cross-cycle findings codified at the Path C basis verdict cycle SEAL boundary

### §38.1 The first authorized run is itself the integration test — a mocked end-to-end PFR is necessary but not sufficient

The Path C pre-fire review (PFR) ran the verdict pipeline end-to-end with a **mocked** engine (non-degenerate synthetic equity) and passed — yet the first **authorized** real run crashed on a degenerate (0-trade / flat) forward equity the mocks never produced, and the real **ingestion** run surfaced a Binance-Vision header-row format drift the headerless fixtures never exercised. A mocked review proves wiring + logic + LOCK-conformance, NOT real-data edge cases (degenerate distributions, source-format drift). This is now the **Path A (integer-floor sizing) / Path B / Path C (degenerate-equity + header-parse) pattern across three cycles**: the authorized run is the real integration gate, and an **instrument-repair-and-re-run on its first failure is clean iff the fix is LOCK-untouching, disclosed, and results-blind** (the partial values seen during the authorized run are the verdict computation Charlie fired, not a no-peek violation; the fix must not be informed by them). **Rule:** treat a mocked end-to-end PFR as necessary-but-not-sufficient; expect the first authorized run to surface real-data integration bugs, and handle them as disclosed instrument repairs (params frozen) rather than as a reason to distrust the result.

### §38.2 Real-data source-format drift defeats fixture-based TDD — mirror the most ROBUST existing parser, not the simplest sibling

Binance Vision added a header row to newer monthly kline CSVs; the Path C kline parser (built mirroring the headerless *funding* parser) assumed headerless, read the header as data, and **silently dropped ~4 years of data** until the real A6 run caught it. The existing OHLCV `bulk_download.py` *already* had header-autodetection — the new parser should have mirrored the **more defensive** existing parser, not the simplest sibling. Fixtures encode the format known at authoring time; a source whose format varies across time will drift. **Rule:** when ingesting from a source with known cross-time format variability, mirror the most defensive existing parser (header-autodetect), and treat the first real ingestion run as the format-validation gate (a silent month-drop is the failure mode to guard against).

### §38.3 A redundancy/agreement diagnostic must guard against VACUOUS agreement; a fenced label is read against the promotion gate, never standalone

Path C's dual-orthogonalization (D2: basis-gated vs funding-gated) tested the basis≈funding cross-frequency redundancy. H2's D2-agreement came via an **inert D1** (the basis gate contributed ~nothing over the no-basis baseline) → `redundancy_read` correctly returned **`vacuous`**, NOT `redundancy_confirmed`: mutual agreement when both gates are inert is vacuous (both merely track the dead price-trend leg). So the cross-frequency localization rests on the **measured losses + mechanism-sanity** (§37.3 / §9), NOT on a conjunction-confirmed redundancy — the verdict prose must say "redundancy plausible/consistent, **NOT confirmed**." Correspondingly the fenced `basis_adds_signal` labels (H1/H3) were **artifacts** (H1 less-bad-losing −0.87 vs funding −1.62; H3 a 0-trade-flat book "beating" a down-market baseline by not trading), NOT edges — confirmed by 0/3 Tier-5. **Rule:** a redundancy/agreement diagnostic must carry an inert-gate guard (agreement under joint-inertness = vacuous, licenses no redundancy claim), and a fenced diagnostic label (`basis_adds_signal` / `redundancy_confirmed`) must ALWAYS be read against the promotion gate (Tier-5 + DSR), never standalone — else a losing or degenerate book's "signal" is mis-read as an edge.

### §38.4 The pre-fire PFR is a distinct, load-bearing gate beyond per-task review — trace the END-TO-END gated entrypoint, not just the modules

Path C's per-task reviews verified each MODULE correct, but the pre-fire PFR (holistic end-to-end + adversarial) caught the gated **run-script** wiring gaps — θ-resolution never invoked in the production path, `basis_marginal=None` (the dual-diagnostic uncomputed), the F3 headline caveat absent — that the per-task reviews missed *because they verified modules, not the end-to-end gated entrypoint*. Across the arc EVERY review layer caught a distinct real bug the prior missed: per-task (the `input_source` contract-widen blocker; the H1-time-stop parity bug); the comprehensive phase-boundary review (the Codex-CRITICAL duplicate-`open_time_utc` merge multiplication; the EWM-NaN null-policy); the PFR (the run-wiring gaps; the H2 de-risk-occupancy wrong-quantity); the authorized run itself (the degenerate-equity crash). **Rule:** keep the layered review (per-task → comprehensive phase-boundary → pre-fire PFR → verdict 2-leg-B2) on a verdict-producing harness; the PFR specifically must trace the END-TO-END gated entrypoint ("does the run, as wired, produce a correct verdict?"), not merely re-confirm the modules pass their unit tests. (Reaffirms + extends §37.4 reviewer-leg specialization.)

### Cross-references
- §37 Path A crypto-native-axis findings — the parent set; §38 extends it with the Path C basis-axis findings (authorized-run-as-integration-test, format-drift-defeats-fixtures, vacuous-redundancy-guard, PFR-as-distinct-gate). §37.1 (gate at the function boundary) **held** — the `PHASE_D_AUTHORIZED` + injected-`_run_backtest` gate worked exactly as designed across Path C's transient-flip fire and its degenerate-crash failure.
- [[project_alpha_source_binding_constraint]] — now **thrice-earned**: OHLCV (Path B), funding (Path A), and the funding/basis premium at 1h (Path C) all "add no rescue under their grids." Cross-frequency, NOT family-level — open interest (OI) is the Charlie-authorized (2026-06-01) next test.
- §3.3 commit-order anti-hindsight — the Step −1 LOCK + the R1 ratification clarification, both committed before the forward_2026 fire.

---

## §39 Cross-cycle findings codified at the Path D open-interest verdict cycle SEAL boundary

Codified at the Path D OI-axis mechanism-first mine verdict cycle SEAL (2026-06-02) — Cycle 4 of the alpha-source falsification series and the **first on a genuinely-INDEPENDENT axis** (OHLCV/funding/basis were correlated). VERDICT: `process_refuted_for_this_grid` (earned negative, **SUBSTANTIVE** — all 3 legs measured losses H1 −0.75 / H2 −2.51 / H3 −3.55; 0/3 Tier-5; 0/3 DSR).

### §39.1 Testing a DIRECTIONLESS axis: gate-not-originate + a velocity firewall on the RIGHT unit

Open interest is **unsigned** (total contracts outstanding), unlike the SIGNED funding/basis premium — so it cannot originate a long/flat direction. The reconciliation that keeps it both expressive and interpretable: OI never fabricates direction (direction = 100% the inherited price-trend cross); OI enters only as one removable boolean gate AND-composed with the price-long, so the D1 vs-momentum baseline is a clean single-factor subtraction. OI's directional "shot" is bought at the *mechanism-of-the-gate* level — H2/H3 gate on OI **VELOCITY** (causal EWM of OI log-change = the flow of new positioning), NOT level (level is near-cointegrated with cumulative price drift → momentum-in-disguise). The firewall has a load-bearing **UNIT** choice: the signal series must be **CONTRACT-denominated** open interest, NOT USDT-notional — because `Δlog(notional) = Δlog(contracts) + price_return`, so a notional-velocity gate mechanically embeds the price return and defeats the firewall (advisor-measured: the two velocities differ on 97% of bars). **Rule:** to test a directionless signal in a directional engine, make it a removable gate on a pre-registered directional source (never let it originate a sign), gate on the signal's FLOW (velocity) not its momentum-cointegrated LEVEL, and pin the firewall-correct UNIT — verify the chosen unit's velocity does not mechanically contain the price return.

### §39.2 §37.3's substantive-vs-vacuous distinction must be a COMPUTED, escalation-gating field — pre-hardened when under-power makes a vacuous outcome plausible

§37.3 requires an all-under-floor `n_tier5_pass=0` earned-negative to separately report whether it is SUBSTANTIVE (≥1 measured forward loss) or VACUOUS (eligibility-exclusion only) and hedge. Path D's heightened under-power (an 8-month-shorter OI history + the §39.3 percentile-NaN holes) made an entirely-vacuous outcome a plausible MODAL result, so the Phase-C 2-leg-B2 flagged — correctly, before any data — that the harness must COMPUTE `negative_is_vacuous_only` vs `negative_has_substantive_basis` (count of legs with `holdout_sharpe < 0`) and GATE the escalation advisory on substantive basis, not leave the distinction in prose. The authorized run then showed all 3 legs were measured losses (substantive, NOT vacuous), so the safeguard was not load-bearing THIS time — but pre-hardening it was correct: a verdict harness must not be *able* to label an entirely-vacuous (untested-because-under-powered) negative as "the axis is dead + escalate." **Rule:** when a cycle's pre-disclosed under-power makes a vacuous outcome plausible, harden §37.3's substantive-vs-vacuous read into a COMPUTED field that GATES the escalation advisory (vacuous-only → not-warranted + hedged verdict) BEFORE the fire — even if the run then proves substantive.

### §39.3 A glitch VALUE in a rolling-window factor propagates a whole-window NaN hole; disclose the coverage-NaN as a power statistic and verify the GATE window is clean

The OI metrics carried 43 zero-OI glitch bars (~0.085%). At the factor layer the right handling is NaN-the-glitch (a zero level must not become a spurious all-time-low percentile; a zero breaks log-change) — applying the project's flag-don't-fail zero-volume-bar discipline to a SIGNAL series (validate-layer: zero → warning not error; factor-layer: zero → NaN). But a single NaN in a rolling-2160 (~90-day) percentile NaNs the ENTIRE window containing it — so 43 scattered zeros NaN'd ~48% of 2024 and ~75% of 2025 (validation/test), a far larger eval-power hit than the raw glitch count suggests. The verdict-deciding forward_2026 window was 0% NaN (the last zero cleared ~90 days before the gate). **Rule:** treat a glitch VALUE at the consumption layer (NaN it; flag-don't-fail at validation), and recognize that a glitch inside a rolling window propagates a whole-window hole — measure + disclose the resulting coverage-NaN fraction per regime as a power statistic (pre-result, anti-hindsight), and explicitly verify the verdict-DECIDING window is clean.

### §39.4 On an INDEPENDENT axis the derived-from redundancy diagnostic (D2) evaporates — drop it, don't repurpose the socket; and match each guard to the threat it can actually detect

Path C's D2 (basis-gated vs funding-gated) was a *derived-from REDUNDANCY* test, legitimate only because basis ≈ funding by construction. For the genuinely-independent OI axis there is no derived-from relation, so D2's motivating question evaporates — the cycle DROPPED D2 entirely (D1-only) rather than repurpose the harness socket to a cross-family or placebo comparison (a diagnostic earns its place by the inference it serves, not by socket availability; the §38.3 inheritance narrowed to "fenced-label-read + inert-D1-modal", and the vacuous-agreement truth-table became unreachable). Relatedly, the build's OI-vs-OHLCV coverage guard was adjudicated (2-leg B2) NOT to enforce a per-bar "misalignment RAISES": a row-coverage guard cannot detect a value-MISLABEL shift (full coverage, wrong values) and would false-positive on the 22 real OI gaps — per-bar causal integrity is structurally enforced by the causal 5min→1h downsample (row N uses only obs < N+1h) + the exact `validate="one_to_one"` join, not by a coverage count. **Rule:** a diagnostic/guard earns its place by the inference or threat it actually serves — drop a derived-from redundancy diagnostic on an independent axis rather than repurpose it, and match a guard to the failure mode it can detect (a row-coverage guard guards coverage, not value-integrity; the causal transform + exact-key join are the per-bar protection).

### Cross-references
- §37 + §38 — the parent crypto-native-axis findings; §39 extends them to the first INDEPENDENT axis (OI). §37.1 (gate at the function boundary) **held again** (the `PHASE_D_AUTHORIZED` + injected-`_run_backtest` gate worked across the transient-flip fire); §38.1 (authorized-run-as-integration-test) held but surfaced **no new crash** this cycle — the real-data surprises were front-loaded into Phase A ingestion (header-CSV format, exact-duplicate 5-min rows, zero-OI bars); §38.4 (PFR/B2 traces the gated entrypoint) caught the **contamination-set-computed-but-unwired** gap before the fire.
- [[project_alpha_source_binding_constraint]] — now **earned across an INDEPENDENT axis** (OI), graduating from "thrice-earned on correlated axes." Scoped honestly: *"OI adds no directional RESCUE to a single-asset long/flat price-trend book under THIS grid"* — NOT family-level (liquidations untried), NOT "OI dead" (cross-sectional/multi-asset RANK — OI's strongest literature edge — is structurally inexpressible in a single-asset engine; the engine-capability gap = the registered [[post-pathd-strategic-fork]]).

## §40 Cross-cycle findings codified at the post-Path-D strategic-freeze cycle SEAL boundary

Codified at the post-Path-D strategic-freeze cycle SEAL (2026-06-02) — the close of the alpha-source falsification arc (Paths B/A/C/D). After the four single-asset earned-negatives, a feasibility spike DEFERRED the cross-sectional fork and an **Evaluation-Gate Power Study** (analytical MDE + Monte-Carlo empirical validation) established that the project's evaluation gate is **CONFIRMATION-LIMITED** under all achievable designs. Charlie registered a **FREEZE (accept-the-negative), `p` low**. The standing alpha-source-binding thesis gains a **SECOND binding constraint: confirmation-power.**

### §40.1 Refutation-powered ≠ confirmation-powered — measure the gate's MDE before concluding "no edge"

A pre-registered single-shot gate (here: DSR-FWER, immutable touch-once splits, ~105-day OOS) that cleanly KILLS large effects can be structurally UNDER-POWERED for the *modest* edge you actually want to deploy. The four earned-negatives all had large effect sizes (−2.5 to −8.4) and were trivially detected — but the gate's 80%-power minimum-detectable Sharpe is ~4.63 (N\*=1) / ~6.22 (N\*=3) annualized, 3-4× above the ~1-1.5 band of a deployable edge (the Monte-Carlo measured a real 1.5-Sharpe edge clearing the live gate only ~20%/~5% of the time — severely under-powered, not literally blind). Concluding "no edge exists" from these negatives is the **category error** of inferring absence-of-edge from absence-of-*confirmable*-edge — the gate was never run on an experiment in-scope for the target effect size. **Rule:** before sealing a "no edge / the frame is exhausted" conclusion, COMPUTE the gate's 80%-power MDE; if the deployable-edge band sits below it, the negatives falsify only large edges and the conclusion must be scoped accordingly (the gate is *refutation*-powered, not *confirmation*-powered). This is a property of the GATE (window length × cost × multiplicity), not of any signal — it transfers to every future cycle on the same gate.

### §40.2 The option-value of an unspent touch-once holdout dominates marginal gate-sensitivity

When a longer/denser OOS would raise the gate's confirmation power, the temptation is to spend a held-out window (validation/test) to buy it. But a touch-once holdout is consumed AT REGISTER TIME (before any candidate clears the redesigned gate), so spending it is a CERTAIN, IRREVERSIBLE cost paid up-front for an option whose underlying (a confirmable edge) has shown no positive evidence. After four clean negatives the prior on such an edge is low, and the redesigned gate's best case is itself fragile (here the lone BUILD-viable arm required BOTH spending 2024+2025 AND abandoning multiplicity, and was Gaussian-only — it slips to INDETERMINATE under heavy tails). **Rule:** do not spend the last clean holdout to chase a confirmation of an edge for which there is zero positive evidence; preserve it UNSPENT as the option value any *fundamentally different* future frame would need. A non-destructive power path (let the fresh forward tape accrue; pool across cycles; switch to a continuous-evidence gate) usually dominates the destructive one.

### §40.3 Empirically validate an analytical power projection before sealing a strategic conclusion on it — and review the validation DESIGN before running it

An analytical power result (a closed-form MDE) is a PROJECTION, not a measurement. Before sealing a consequential strategic conclusion (here: a freeze) on it, validate it empirically — run a Monte-Carlo of known-true-Sharpe series through the LIVE gate and measure the actual detection rate (it should land at the analytical 80% at the MDE, and far below 50% at the deployable Sharpe). Use a real-data arm (bootstrap of the actual return distribution) as the load-bearing test, separate from an iid self-consistency arm. The discipline paid off twice this cycle: the **design-review gate caught a fatal DGP bug PRE-RUN** (a re-meaning recipe that collapsed the Sharpe-estimator sampling spread `sd(sr_hat)` from ~1/√(T−1) to ~2e-17 → a falsely-confirmatory step function), and a **diagnostic caught a second bug** (the bootstrap sampled only the first T bars = a single 2020-COVID volatility window, halving the recovered Sharpe). **Rule:** validate-before-seal for any analytical projection a strategic decision rests on; review the validation DESIGN before running it (a power harness can be wrong in a *falsely-confirmatory* direction — a design-review gate is high-value precisely there); and pin each fixed bug with a regression test (e.g. `sd(sr_hat) ≈ 1/√(T−1)`; injected-Sharpe recovery; resampler↔primitive equivalence).

### §40.4 Accept-the-negative must be precisely scoped + non-foreclosing

A "stop / accept-the-negative" is an earned finding only if it is scoped to *what was actually tested* and does not foreclose what was not. Scope it explicitly: the negative falsifies the *tested* expression (here: single-asset, long/flat, four signal axes, on the achievable OOS), NOT the abstract claim ("no edge exists" / "the asset class is dead"). Record the UNTESTED moves as **untested-not-exhausted** (here: short legs, continuous sizing, on-chain flow, cross-sectional rank), distinct from anything EARNED-EXCLUDED (here: liquidations, per the spanning + Binance-throttle data-integrity argument). Preserve optionality (the unspent holdout; the deferred-open non-destructive paths) and pre-register nothing as the authorized successor (anti-pre-emption — a freeze authorizes no next cycle). **Rule:** a freeze is a stop on *this frame as disciplined*, not a foreclosure of the space; the honest closeout names the two binding constraints, the precise scope, and the deferred-open paths — each a fresh future register.

### Cross-references
- §37 + §38 + §39 — the crypto-native single-axis falsification findings (funding / basis / OI); §40 closes the arc they belong to by establishing the *second* (evaluation-gate) binding constraint that the four earned-negatives sit behind. §39's "earned across an INDEPENDENT axis" localization is the alpha-source half; §40 adds the confirmation-power half.
- [[project_alpha_source_binding_constraint]] — now a **two-constraint** thesis (alpha-source AND confirmation-power); the freeze is the earned acceptance the arc was always building toward.
- [[post-pathd-strategic-fork]] — the cross-sectional fork was DEFERRED (not chosen) on power + cost + decayed-prior grounds; it remains a deferred-open path, not the authorized next register.
- Decision record: `docs/phase5/POST_PATHD_FREEZE_NOTE.md`; study: `docs/phase5/EVAL_GATE_POWER_STUDY.md`; harness: `backtest/eval_power.py` + `backtest/eval_power_mc.py`.

## §41 Findings codified at the B-iii continuous-evidence / Bayesian eval-layer scoping SEAL boundary

Codified at the SEAL of the B-iii scoping cycle (2026-06-02), the first exploration of a post-Path-D-freeze **deferred-open path** (path (ii): a pre-registered Bayesian / continuous-evidence evaluation layer). A pure-analytical scoping (purpose: a go/no-go BUILD/CONDITIONAL/DON'T-BUILD verdict) adjudicated seven candidate mechanisms (M1 posterior-sizing, M2 Bayesian sequential, M3 cross-cycle pooling, M4 skeptical-prior reporting, M5 always-valid/e-values, M6 change-the-estimand, M7 within-batch empirical Bayes) against a pre-registered two-axis rule + two-prong anti-motivated clause. **Verdict: CONDITIONAL-BUILD (Charlie-accepted)** — M3/M1 killed, M2/M4/M5 add only time/honesty, but **M6 (a directional/sign/Brier estimand) plausibly raises confirmation power at BTC's heavy tails**, gated CONDITIONAL by a deployment-relevance question. The verdict was CORRECTED DON'T-BUILD→CONDITIONAL when the result-B2 caught a verdict-flipping bug.

### §41.1 A cheap analysis can REVERSE a "no-free-lunch" prior — reason-then-compute, and let the compute flip the sign

The scoping's governing premise was correct and important — no statistic manufactures statistical power on a fixed window (the MDE wall is set by T × N*). The trap is treating that premise as the *answer*. Reasoning from it predicted DON'T-BUILD for every mechanism; *computing* each mechanism surfaced a genuine exception (M6): a change of estimand does not add power on the *same* statistic, but a *different* statistic (the sign/median test) has *different* power, and under heavy tails it is HIGHER (the classic median-beats-mean-under-heavy-tails efficiency, ARE = (2·f(0))² crossing 1 at raw kurtosis ≈ 11.84, near BTC's realized γ4). **Rule:** a sound no-free-lunch premise bounds *one* statistic's power, not the *choice* of statistic/estimand — run the cheap computation per mechanism rather than letting the premise pre-decide the verdict. (Same shape as §40.3's validate-before-seal, applied at the per-spot-check level: the computation surfaced what the framing got backwards.)

### §41.2 On a NUMERIC result, the 2-leg B2's load-bearing value is the INDEPENDENT recompute — not the methodology pass; and a self-disclosed "I didn't verify X, leg-2 better-positioned" is a high-value handoff

The first-pass SV6 concluded the directional estimand has LOWER power (it modeled the heavy-tail effect on the *Sharpe* side via Mertens — O(sr²)≈0 at a modest edge — but held the *sign test's* density f(0) at its Gaussian value). The advisor leg rated SV6 "sound" on the methodology lens **but explicitly disclosed it had not verified the sign-vs-t ARE against a primary source and named the cross-model leg better-positioned**; Codex's independent recompute then caught the bug as **verdict-flipping** (crossover ≈ 11.84, not the module's ≈ 8892). **Rule:** for a result whose verdict rests on a computed number, the decisive B2 contribution is an *independent recomputation from first principles*, not a methodology-level "looks sound"; weight a leg's reproduced numbers above its bottom-line rating; and treat a reviewer's honest "I could not verify X — leg-2 is better-positioned" as a *routing signal to be acted on*, not a hedge to be discounted. (Mirror of the reverse-direction-catch pattern: here the cross-model leg caught what the senior-quant leg's methodology pass missed.)

### §41.3 Statistical power ≠ deployment relevance — a more-powerful estimand can confirm the WRONG quantity

M6's directional estimand genuinely raises *statistical* power under heavy tails — but the quantity it confirms is a **median / directional shift** (the strategy wins > 50% of bars), which is **not** a deployable **Sharpe** edge: a high hit-rate with negative skew ("pick up pennies, lose big") has a *negative* Sharpe, and the cost gate compounds it. A test can therefore be strictly more powerful AND confirm something undeployable. **Rule:** never read "higher-power test" as "confirms a deployable edge"; a confirmation gate must be powerful *for the deployment-relevant estimand* (risk-adjusted, net-of-cost, skew-aware), not merely powerful. When a power gain and a deployment-relevance gap coexist, that is the precise signature of a CONDITIONAL verdict (the power lever is real; the legitimacy-for-confirmation needs a separate quantification) — not a BUILD and not a DON'T-BUILD.

### §41.4 A pre-registered PINNED decision criterion does its job at verdict time — it resolves a soft hinge mechanically and forces a verdict change to rest on substance

The §5 rule pinned the Axis-2 `high`-vs-`modest` line at the written-spec B2 *specifically* so the one soft cell (M5: pre-register-the-tracker-now-while-clean) could not be decided by wording. At verdict time the pin did exactly that — applied literally, M5's anytime-valid monitoring changes no current action on a frozen frame with no live candidate (the per-cycle pre-registration discipline already supplies the clean window) → `modest` → not the verdict driver. So when the verdict moved to CONDITIONAL, it moved for a *substantive* reason (M6's measured power result), not because a hinge was talked into `high`. **Rule:** pin the verdict-bearing soft cell of a decision rule BEFORE the analysis; a pin is not bureaucracy — it is what lets you tell "the verdict changed because the numbers changed" apart from "the verdict changed because I re-described a judgment."

### Cross-references
- §40 — the post-Path-D freeze established the *confirmation-power* binding constraint and recorded path (ii) (a Bayesian/continuous-evidence layer) as deferred-open; §41 is the SEAL of the first exploration of that path. Outcome: the path is NOT closed (a live M6 lead remains, CONDITIONAL), and the freeze stands (a CONDITIONAL scoping does not un-freeze).
- §40.3 — validate-before-seal at the strategic level; §41.1/§41.2 are its per-mechanism / per-number instantiation (the computation, and the independent recompute, catch what the framing/methodology-pass got backwards).
- [[project_post_pathd_strategic_fork]] / [[project_alpha_source_binding_constraint]] — the two-constraint thesis; B-iii's CONDITIONAL keeps the Bayesian-layer path open via an estimand change, ENDORSED-not-authorized.
- Verdict memo: `docs/phase5/BAYESIAN_EVAL_LAYER_SCOPING.md`; spec: `docs/superpowers/specs/2026-06-02-bayesian-eval-layer-scoping-design.md`; harness: `backtest/eval_layer_scoping.py`.

## §42 Findings codified at the estimand-deployability confirmation SEAL boundary

Codified at the SEAL of the estimand-deployability confirmation (2026-06-02), which closed the B-iii **M6 lead** (change the evaluation estimand from Sharpe to a directional/Brier statistic) as an **earned DON'T-BUILD**. The design-point 2-leg converged that the verdict was near-foreseeable; Charlie registered a **minimal-confirmation run** of four pillars (P1 skew-divorce; P2/P3 net-benefit-after-2N\*-multiplicity < 10pp; P4 runs-test = confirmation-layer category error). Result: DON'T-BUILD, no surprise — graduating the *whole continuous-evidence / Bayesian / alternative-estimand evaluation-layer family* to exhausted as a build-now confirmation-power lever (reinforcing, not weakening, the post-Path-D confirmation-power freeze).

### §42.1 When a design-point review converges on a near-foreseeable negative, run a MINIMAL confirmation — not a full study, and not accept-without-run

A 2-leg design-point review can converge strongly that a study's verdict is foreseeable (here: three of four pillars were reproduced math at the design point). The temptation is to either (a) run the full registered study anyway (over-investment) or (b) accept the verdict without any run (skips the rigor + forecloses a surprise). The right move is a **minimal-confirmation run**: pre-register the few determinate pillars + a genuinely-reachable surprise branch, compute them cheaply, and let the verdict be earned. This **costs a fraction of a full study yet preserves the chance of a surprise** — and the precedent shows the surprise is real: B-iii's analysis *flipped* DON'T-BUILD→CONDITIONAL on exactly such a run, while this run did *not* flip — both outcomes vindicate running over accepting. **Rule:** "the reviewers agree it's foreseeable" justifies *scoping down*, never *skipping*; a minimal confirmation is the cheapest artifact that both earns the negative and keeps the surprise branch live.

### §42.2 Foreclose "did the coarse grid hide a surprise?" with a DENSE out-of-grid global-max search — persisted in the harness, not a monotonicity argument

A negative verdict computed over a pre-registered grid invites the false-negative objection: *a finer or wider grid might hide a region that flips it.* Close it directly. Search a **dense box far outside** the registered grid (here: 3960 cells = 40 Sharpe ∈ 0.05–2.0 × 11 kurtosis ∈ 4–1000 × 9 N\* ∈ 1–500 — every dial cranked toward the BUILD outcome, most settings absurd/non-deployable) and report the **global** extremum (here: net benefit 7.09pp at Sharpe 2.0/γ4 1000/N\*=1 — still below the 10pp tripwire). The dense global maximum is the robust foreclosure: a between-grid peak exceeding the tripwire would require a sharp spike that a smooth decision quantity sampled this densely cannot hide. **Two cautions the SEAL-eve B2 surfaced (both load-bearing):** (1) do **NOT** lean on a *monotonicity* argument — it is tempting ("net benefit rises toward the BUILD-favorable corner") but here the quantity is **not globally monotone** (it *decreases* in Sharpe at high N\* / low kurtosis), so a blanket monotonicity claim is a falsifiable overclaim; the dense global max stands without it. (2) **Persist the search in the harness** (a function + a test + the regenerated artifact), not in reviewer prose — a standing methodology principle codified on a computation that lives only in a review transcript is not reproducible by a future reader. (This was the reviewer move that resolved a flagged over-correction worry **on evidence, not on prior** — the bias-guard working — and the SEAL-eve catch that kept the evidence honest and durable.)

### Cross-references
- §41 — the B-iii cycle that produced the CONDITIONAL M6 lead; §42 is the SEAL of the confirmation that closed it (DON'T-BUILD). Together §41+§42 exhaust the evaluation-layer family as a build-now confirmation-power lever; the freeze's *other* deferred-open paths (fresh-OOS accrual; structurally-different frames) remain untouched and the holdout UNSPENT.
- §40 — the confirmation-power binding constraint; this DON'T-BUILD reinforces it (no evaluation re-framing rescues confirmation on the current frame).
- Verdict memo: `docs/phase5/ESTIMAND_DEPLOYABILITY_CONFIRMATION.md`; spec: `docs/superpowers/specs/2026-06-02-estimand-deployability-confirmation-design.md`; harness: `backtest/eval_estimand_deploy.py`.
