...
**Cross-validation register at simplified MVD:** per-candidate scalar consumed from audit_v1 holdout_summary.json + cross-checked against `holdout_results.csv` aggregate CSV row for same hypothesis_hash. If per-candidate scalar disagrees between JSON and CSV by any non-trivial amount (|delta| > 1e-9 floating-point precision), candidate excluded from primary computation; discrepancy documented at result register.
...
1. **Bonferroni-style threshold (existing `evaluate_dsr.py` precedent):** `SR_threshold_Bonferroni = sqrt(2 * ln(N))` at N=198. Threshold value ≈ 3.0688 at N=198.
...
**Pass threshold (signal evidence at simplified register):** primary screen p-value < 0.05 at N=198 OR maximum observed Sharpe > Bonferroni threshold sqrt(2 × ln(198)) ≈ 3.0688. Both criteria reported; primary disposition = pass if BOTH cross-validate (conservative AND-gate at register-precision register).
...
At N=198: ≈ 3.0688. Observed `SR_max > SR_threshold_Bonferroni` indicates signal-distinguishable at Bonferroni heuristic register.
...
5. **JSON-vs-CSV cross-validation failure (per §3.4):** if scalar disagrees between holdout_summary.json and holdout_results.csv aggregate row by |delta| > 1e-9, candidate excluded; discrepancy documented.
...
**α-Refine deliverable extension (per advisor concern):** Step 1 deliverable additionally documents the empirical multi-run hypothesis_hash distribution at experiments.db (246 runs > 198 distinct hashes at bear_2022 partition; characterize empirical distribution at audit-trail register for forward-looking documentation). Note: this is descriptive register only at simplified MVD scope; cross-register mapping to experiments.db is out-of-scope per §2.4.
