...
- **V9-E1 (Gap 1 note fix):** the Step 14.6 note "Note: data artifacts under `data/` are gitignored" (located by that quoted text) is FALSE → replace with: the canonical + archive under `data/phase2c_evaluation_gate/` ARE git-tracked (only `data/raw`,`data/features`,`data/quality`,`data/results`,`data/compiled_strategies`,`raw_payloads`,`*.db` are gitignored); per Q2=(a) the recovered canonical + archive are committed at Steps 14b.1.5 + 14b.2.7. Remove the "(gitignored)" label from the ratify-packet artifact-table canonical + archive rows (located by their `data/phase2c_evaluation_gate/...` path strings) and the Step 14c.2 commit-body "Data artifacts produced (gitignored):" line (located by that quoted text); KEEP "(gitignored)" ONLY on the `backtest/experiments.db` artifact-table row (genuinely ignored, `.gitignore:18`). **Do NOT** `git add backtest/experiments.db`. [v9.1 per advisor HIGH-1: located by quoted text / table-row identity; plan-body line numbers intentionally OMITTED — they drift with every edit (incl. this errata's own inserts), so quoted-text is the durable locator.]
...
- **V9-E5 (scope + file-table):** the **Sub-plan scope** line ("NO new source code edits ... adds ONE new test file ... + ONE new fixture ... + ONE new evidence artifact"; located by that quoted text; currently `plan:7` — line 5 is now the v9 supersession pointer this errata inserted) → amend to register `tests/test_t1_4_backward_compat.py` as MODIFIED (pc9 BASELINE + A3/A4/A5 rescope) and the canonical + archive dirs as git-tracked content COMMITTED at 14b.1.5/14b.2.7. Update the file-structure table accordingly.

### §V9.6 — DESIGN INVARIANT: no full-suite run while the canonical is absent
`_per_candidate_dirs()` (`tests/test_t1_4_backward_compat.py:96-99`) is called at **COLLECTION time** inside the A1/A6 (`:205`) and A5 (`:385`) `@pytest.mark.parametrize` decorators → it runs `_PHASE4_DIR.iterdir()` during collection. (A2's `_load_legacy_summary` at `:235` also reads the canonical, but only inside test BODIES — it fails at RUNTIME, not collection; the A1/A6/A5 collection-time crash alone is sufficient for this invariant. v9.1 ADOPT: convergent advisor MED-1 + Codex L1.) If the canonical is ABSENT — the entire window between Step 14.1 fire (W3 vacates the canonical, 14.2e) and Step 14b.1 mv (repopulates it) — `iterdir()` raises `FileNotFoundError`, which **crashes the whole pytest collection** AND the pc9 gate's own subprocess `_collect_count([])`. Therefore: **NO full-suite `pytest -q` may run while the canonical is absent.** The v8 "Expected: 2374 passed" at Step 14.5 is STRUCTURALLY IMPOSSIBLE and is DELETED (not relocated as-is); the relocated full-suite at Step 14b.2.8 is the only valid zero-regression gate, and it MEASURES the count (current-HEAD-collected + 12 V4).

### §V9.7 — VERIFIED-CLEAR (do not re-litigate at re-review)
(a) B1 4-tuple test (`test_count=49`, `dynamic_count=23`; locked-tuple values at `tests/test_t1_4_backward_compat.py:86-88`) unaffected — V4 adds zero `_write_to_registry` call-sites. (b) `test_per_candidate_count_locked_at_39` (`:340`) + A5 parametrize stay at 39 (new parquet is a FILE; helper filters `is_dir()`). (c) The `7c8f4a7` byte-identity assertion has exactly ONE consumer (t1_4 `_verify_byte_identity`) — the rescope is complete. (d) `backtest/experiments.db` correctly stays gitignored (`.gitignore:18`). (e) No CI/.github workflow reads the canonical (none exist / no matches).
...
### §V9.9 — v9.1 polish (B2 re-review ADOPT, 2026-05-29)

Plan v9 → v9.1 per the first B2 re-review (Codex Leg-B + advisor Leg-D, parallel on commit `cb59a55`); **BOTH APPROVE-WITH-FINDINGS; both independently confirmed v9 SOUND for re-fire** (0 BLOCKING either leg; Codex 0 HIGH/MEDIUM; advisor 0 BLOCKING). 3 doc-precision ADOPT applied inline (NO change to the §V9.4 sequence, the edits' substance, or soundness):
- **(1) CONVERGENT (advisor MED-1 + Codex L1):** §V9.6 reworded — the collection-crash is caused by the A1/A6/A5 parametrize `_per_candidate_dirs()` call (collection time); A2's `_load_legacy_summary` fails at RUNTIME, not collection. Conclusion unchanged (A1/A6/A5 crash alone suffices).
- **(2) advisor HIGH-1:** V9-E1/V9-E5 stale line citations re-anchored to **quoted-text / table-row locators**; fragile plan-body line numbers DROPPED rather than chased — they drift with every edit (this §V9.9 insert itself shifted the gitignored note from a prior `:2332` to `:2345`, re-confirming the advisor's warning), so quoted-text is the durable locator (invariant-level closure per `feedback_invariant_level_vs_enumeration`). Stable refs retained where they don't drift: scope `plan:7`; test_t1_4 `:86-88`/`:205`/`:385`. Orchestrator grep-verified every locator.
- **(3) advisor LOW-1:** §V9.7(a) B1 citation `:517`→`:86-88` (the locked-tuple values).

**PUSHBACK — Codex V2** (claimed the plan references `tests/backtest/test_v4_evaluation_gate.py` "in one place"): orchestrator `grep -n "test_v4_evaluation_gate\|tests/backtest/"` on the plan returned EMPTY — no such reference exists; the only test-file path is `tests/test_b_c_narrow_v4_reproducibility.py` (correct). Adjudicated **HALLUCINATION → no action** (citation-verification discipline; B2 reverse-direction + Layer-3 cross-check working as designed).

**No-action:** Codex N1 (stale v8 *body* text — superseded by the top supersession pointer + §V9.4/§V9.5 authoritative; no action under the in-place-amendment convention) + advisor LOW-2 ("+39 parquet" framing is forward-looking but correct in the post-fire-commit context).

**Rule-2 SEAL-eve (Codex, lightweight) — CLEAR** (verified on commit `0025afd`): all 3 ADOPT confirmed correctly applied + 0 new defects (§V9.6 reword accurate — A2 confirmed runtime-only; V9-E1/V9-E5 quoted-text locators resolve; §V9.7(a) B1 `:86-88` correct; §V9.9 changelog accurate). **v9.1 SEALED (re-fire-ready) per Charlie re-SEAL register 2026-05-29.** Full v9 patch cycle converged: v9 B2 (Codex+advisor both APPROVE-WITH-FINDINGS / SOUND) → v9.1 (3 doc-precision ADOPT + Codex-V2 hallucination PUSHBACK) → Rule-2 SEAL-eve CLEAR. **Next register-event: Charlie #N+19a′** — re-fire authorization (Task 13 RED re-commit + T13 fire per §V9.4 corrected sequence); NO data fires without it.
...
**Plan version:** v3-Phase3 v9.1 SEALED — re-fire-ready (v8 lineage R1 17 + R2 14 + R3 13 + R4 2 + R5 4 + SEAL-eve R1 4 + SEAL-eve R2 2 inline → v8 SEALED #N+19''''''' SEAL-RATIFY 2026-05-28, then EXECUTED + REVERTED Option A 2026-05-29 → v9 re-fire errata (Gaps 1/2/2b + Q1=a/Q2=a/Option A) → v9.1 B2-polish (3 ADOPT); v9.1 SEALED per Charlie re-SEAL register 2026-05-29 after Codex+advisor B2 APPROVE/SOUND + Rule-2 SEAL-eve CLEAR; PFR R1 17 ADOPT applied per Charlie register #N+19 Path 1 2026-05-28 + PFR R2 14 ADOPT applied per Charlie register #N+19' Path 1 2026-05-28 + PFR R3 13 ADOPT applied per Charlie register #N+19'' Path 1 2026-05-28 + PFR R4 2 ADOPT applied per Charlie register #N+19''' Path 1 2026-05-28 + PFR R5 4 ADOPT applied per Charlie register #N+19'''' Path 2 2026-05-28)
**Plan path:** `docs/superpowers/plans/2026-05-27-b-c-narrow-phase-3-fire-plan.md`
...
End of Plan v3-Phase3 v9.1 SEALED — re-fire-ready (v8 SEALED → EXECUTED + REVERTED Option A 2026-05-29 → v9 re-fire errata → v9.1 B2-polish; Codex+advisor B2 both APPROVE/SOUND + Rule-2 SEAL-eve CLEAR; Charlie re-SEAL register 2026-05-29. v8 lineage: R1 17 + R2 14 + R3 13 + R4 2 + R5 4 + SEAL-eve R1 4 + SEAL-eve R2 2 inline; Path 2 SEAL-eve direct per Charlie register #N+19''''; PFR R1 ADOPT 17 inline + 3 PUSHBACK adjudications applied per Charlie register #N+19 Path 1 2026-05-28 + PFR R2 ADOPT 14 inline + 3 sub-decision adjudications applied per Charlie register #N+19' Path 1 2026-05-28 + PFR R3 ADOPT 13 inline + 1 Sub-1 reopen-and-ACCEPT applied per Charlie register #N+19'' Path 1 2026-05-28 + PFR R4 ADOPT 1 BLOCKING + 1 LOW absorbed applied per Charlie register #N+19''' Path 1 2026-05-28 + PFR R5 ADOPT 4 inline applied per Charlie register #N+19'''' Path 2 2026-05-28; SKIP standard PFR R6 → dispatch SEAL-eve Rule 2 directly per Phase 2 R7→SEAL-eve precedent).
