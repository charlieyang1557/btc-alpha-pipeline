...
5. **Mid-run instrument-repair disclosure (2026-05-31; required by the small 2-leg B2 before the verdict re-run).** The first verdict-run execution surfaced a *Section-C harness bug*: the ternary `SizingSpec` emitted `order_target_percent`, whose `CommInfoBase.getsize = int(cash // price)` floored sub-1-unit BTC targets to **0 whole units** → all strategies produced **0 trades** (the dead-18 traded only because `full_equity` uses `PercentSizer(retint=False)`, fractional). The localized fix — emit a fractional `self.buy(size = frac × pct × cash / close)`, mirroring `full_equity` exactly (`frac=1.0` is byte-equal to `full_equity`, empirically verified) — is **instrument-repair**: it makes the strategies *tradeable*, a precondition for ANY verdict, and lifts H1/H2/H3 **symmetrically**. The LOCK-frozen elements (H-params, entry/exit predicates, sizing bands, N\*=3, the 15bps gate) are **untouched**; the cost model is **untouched**. Trade counts (119/47/22) and a glimpse of H1's debug Sharpe were seen during fix-confirmation; per the 2-leg review (advisor endorsed "instrument-repair, not reverse-fitting"; Codex stalled in background — adjudicated via the advisor + empirical `frac=1.0==full_equity` + TDD red→green + full-suite-green), **no LOCK-frozen element is or will be changed in response to any pre-verdict value.** This note is committed BEFORE the verdict re-run. (Methodology lesson for closeout codification: test fixtures must use realistic price scales — the Section-C engine test used price=100 and never exercised fractional BTC.)
6. **§9 escalation-prong amendment (2026-05-31; Charlie register, on the record — advisor Finding A).** The §9 A-escalation second prong "the Step-0 re-score lifted **no** dead candidate's net excess Sharpe **above 0**" was pre-registered (LOCK line 44, design spec §9) AND originally coded (`_step0_lifted_any`) as *point-estimate-OR-significance* (`deflated_z_B > 0` OR `pass_B`). Charlie **tightens** it to **DSR-significance only** (`pass_B`): an excess-Sharpe point estimate > 0 that is not statistically significant cannot establish a real edge over large samples (the top Step-0 lift was PSR 0.84 on 5 trades, kurtosis 257 — sampling noise). **This is an explicit AMENDMENT, not a re-reading** of the literal criterion. It is clean: escalation is advisory-by-design (Charlie's authority), it *tightens* the bar (more rigor), it cannot cherry-pick (both readings yield **0 deployable strategies**), and it adds no variant (N\* intact). Under `pass_B`: 0/39 lift → prong satisfied → **A-escalation WARRANTED**. (Warranted ≠ Path A authorized: Path A's data/hypotheses/N\* are unscoped, a separate register — §9/§10 anti-pre-emption.)
