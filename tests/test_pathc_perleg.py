...
"""Path C tiered (24h+72h) mechanism-sanity tests (Task C5).
...
sign at BOTH a 24-bar and a 72-bar horizon on the TRAIN frame (adapts Path A's
_leg_mean_sign to two horizons) and records both horizon signs + the tier.

Retargeted at the 3 BASIS hypotheses (Path C LOCK Pre-registration 1):
  H1 basis_extreme_fade        — sane sign NEGATIVE (reversal DOWN).
  H2 basis_regime_gate         — sane iff permissive_mean > derisk_mean AND
                                  permissive_mean > 0.
  H3 basis_momentum_continuation — sane sign POSITIVE (continuation UP).
"""
...
import backtest.pathc_perleg_mechanism as pm
from backtest.pathc_perleg_mechanism import classify_leg, classify_h2_leg
...
def test_h1_strong_sane_with_crafted_reversal_data():
    # H1 tail population (basis_pct_rank_2160 >= 0.90 AND basis_sign > 0) has
    # NEGATIVE forward return at both horizons -> the crowded-premium fade is sane
    # at both -> strong_sane.
    n = 100
    ...
    df = pd.DataFrame({
        "open_time_utc": pd.date_range("2020-01-01", periods=n, freq="h", tz="UTC"),
        "basis_pct_rank_2160": rank,
        "basis_sign": sign,
        "basis_ewm_480": np.zeros(n),
        "basis_ewm_240_pctrank_2160": np.full(n, 0.5),
