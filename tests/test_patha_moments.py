# tests/test_patha_moments.py
"""Path A moments loader integrity tests (reuses the tier6 integrity gate).

Adapted from tests/test_pathb_moments.py; theme="patha". load_patha_moments
reuses tier6.load_candidate_moments (sha256 + independent gamma3/gamma4/T
recompute), so Path A moments meet the SAME integrity bar as the sealed cohort
...
def test_load_patha_moments_roundtrip(tmp_path):
    rows = [_write_candidate(tmp_path, "h1aaa")]
    df = build_cohort_csv(rows, tmp_path)
    cms = load_patha_moments(["h1aaa"], df, tmp_path)
...
def test_integrity_gate_fires_on_tamper(tmp_path):
    ...
    with pytest.raises(ValueError, match="sha256 integrity mismatch"):
        load_patha_moments(["h1bbb"], df, tmp_path)
