...
def test_producer_writes_dead18_layout(tmp_path):
    ...
    # holdout_sharpe is now RECOMPUTED on the cropped forward-window slice (FIX 3),
...
def test_producer_prepends_warmup_and_evaluates_full_window(tmp_path):
    # FIX 3: the producer must feed the engine PRE-WINDOW history (window_start -
    # WARMUP_BARS bars) so the funding WARMUP_BARS completes BEFORE the forward
