"""Reconcile existing and new OI 1h data into the canonical parquet.

Mirrors ingestion/markprice_reconcile.py for the 1h open-interest series:
1. Archive the current canonical OI file (copy, before overwrite).
2. Load existing + new DataFrames.
3. Concatenate, sort, deduplicate on open_time_utc (prefer binance_vision).
4. Validate the merged result via validate_oi.
5. Save as the new canonical OI file.

OI is a distinct file from OHLCV, funding, and mark-price, so there is no
...
Usage:
    python -m ingestion.oi_reconcile --existing data/raw/btcusdt_oi_1h.parquet --new data/raw/btcusdt_oi_1h_update.parquet
    python -m ingestion.oi_reconcile --existing data/raw/btcusdt_oi_1h.parquet --new data/raw/btcusdt_oi_1h_update.parquet --dry-run
"""
...
from ingestion.validators import save_report, validate_oi
...
def archive_file(file_path: Path, archive_dir: Path = ARCHIVE_DIR) -> Path | None:
    """Archive the current canonical OI file before overwriting.

    Creates a timestamped copy in `archive_dir` (default data/raw/archive/).

    Args:
        file_path: Path to the canonical OI parquet file.
...
def reconcile_oi(
    ...
    """Merge existing and new OI data with source-priority deduplication.

    Args:
        existing_df: Current canonical OI DataFrame.
...
def main() -> int:
    """CLI entry point for OI reconciliation.

    Returns:
        Exit code: 0 on success, 1 on failure.
    """
    parser = argparse.ArgumentParser(description="Reconcile existing and new OI data")
    parser.add_argument(
        "--existing", type=str, required=True, help="Path to existing canonical OI parquet"
    )
    parser.add_argument("--new", type=str, required=True, help="Path to new OI data parquet")
    ...
    merged_df, stats = reconcile_oi(existing_df, new_df)
    ...
    save_report(report, QUALITY_DIR, prefix="oi_reconcile_validation")
    ...
    # Atomic-promote: write to staging, then rename (mirrors markprice_reconcile).
