"""Reconcile existing and new mark-price data into the canonical parquet.

Mirrors ingestion/funding_reconcile.py for the 1h mark-price series:
1. Archive the current canonical mark-price file (copy, before overwrite).
2. Load existing + new DataFrames.
3. Concatenate, sort, deduplicate on open_time_utc (prefer binance_vision).
4. Validate the merged result via validate_markprice.
5. Save as the new canonical mark-price file.

Mark-price is a distinct file from OHLCV and funding, so there is no
cross-venue OHLCV interaction here — only binance_vision (bulk) and
ccxt_binance (incremental) ever appear, and the source priority resolves
any PK conflict.

Usage:
    python -m ingestion.markprice_reconcile --existing data/raw/btcusdt_markprice_1h.parquet --new data/raw/btcusdt_markprice_1h_update.parquet
    python -m ingestion.markprice_reconcile --existing data/raw/btcusdt_markprice_1h.parquet --new data/raw/btcusdt_markprice_1h_update.parquet --dry-run
"""
...
def archive_file(file_path: Path, archive_dir: Path = ARCHIVE_DIR) -> Path | None:
    """Archive the current canonical mark-price file before overwriting.

    Creates a timestamped copy in `archive_dir` (default data/raw/archive/).

    Args:
        file_path: Path to the canonical mark-price parquet file.
...
def reconcile_markprice(
    existing_df: pd.DataFrame,
    new_df: pd.DataFrame,
) -> tuple[pd.DataFrame, dict]:
    """Merge existing and new mark-price data with source-priority deduplication.

    Args:
        existing_df: Current canonical mark-price DataFrame.
...
def main() -> int:
    """CLI entry point for mark-price reconciliation.

    Returns:
        Exit code: 0 on success, 1 on failure.
    """
    parser = argparse.ArgumentParser(description="Reconcile existing and new mark-price data")
    parser.add_argument(
        "--existing", type=str, required=True, help="Path to existing canonical mark-price parquet"
    )
    parser.add_argument("--new", type=str, required=True, help="Path to new mark-price data parquet")
    ...
    merged_df, stats = reconcile_markprice(existing_df, new_df)
    ...
    report = validate_markprice(merged_df)
    ...
    save_report(report, QUALITY_DIR, prefix="markprice_reconcile_validation")
    ...
    # Atomic-promote: write to staging, then rename (mirrors funding_reconcile).
